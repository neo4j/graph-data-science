...
To build your own algorithms using the Pregel API (see at https://neo4j.com/docs/graph-data-science/current/algorithms/pregel-api/#algorithms-pregel-api-example[docs]), we recommend starting with the https://github.com/neo4j/graph-data-science/tree/master/examples/pregel-bootstrap[pregel-bootstrap project].
...
The algorithms are located in the `algo-common` and `algo` modules:
...
The procedures are located in the `proc-common` and `proc` modules:
...
By default we build against the latest stable Neo4j version, which is defined in `public/gradle/neo4j.dependencies.toml`.
