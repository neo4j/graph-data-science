 ...
 * </ul>
 */
public class TraversalToEdgeStrategy implements MultiSourceBFS.ExecutionStrategy {
    ...
    TraversalToEdgeStrategy(Graph[] graphs, BfsConsumer perNodeAction) {
        this.graphs = graphs;
        ...
        while (true) {
            ...
            while (visitCursor.next()) {
                ...
                for (int i = offset; i < limit; ++i) {
                    if (array[i] != 0L) {
                        prepareNextVisit(array[i], base + i, visitNextSet);
                    }
                }
            }
            ...
            if (!hasNext || depth >= graphs.length) {
                return;
            }
