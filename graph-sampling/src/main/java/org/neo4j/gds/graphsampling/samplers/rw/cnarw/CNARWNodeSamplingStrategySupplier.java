...
public class CNARWNodeSamplingStrategySupplier implements NodeSamplingStrategySupplier {
    
    @Override
    public NextNodeStrategy apply(
        Graph graph,
        SplittableRandom split,
        Optional<HugeAtomicDoubleArray> totalWeights
    ) {
        if (totalWeights.isPresent()) {
            return new WeightedCommonNeighbourAwareNextNodeStrategy(graph, split);
        } else {
            return new CommonNeighbourAwareNextNodeStrategy(graph, split);
        }
    }
}
