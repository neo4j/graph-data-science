...
import static org.neo4j.gds.core.pagecached.VarLongEncoding.encodeVLongs;
import static org.neo4j.gds.core.pagecached.VarLongEncoding.encodedVLongSize;
import static org.neo4j.gds.core.pagecached.VarLongEncoding.zigZag;
import static org.neo4j.gds.core.pagecached.ZigZagLongDecoding.zigZagUncompress;
...
final class CompressedLongArray {
    ...
    private void ensureCapacity(int pos, int required, byte[] storage) {
        if (storage.length <= pos + required) {
            ...
            tracker.remove(MemoryUsage.sizeOfByteArray(storage.length));
            tracker.add(MemoryUsage.sizeOfByteArray(newLength));
    ...
    private void ensureCapacity(int pos, int required, int weightIndex) {
        if (weights[weightIndex].length <= pos + required) {
            ...
            tracker.remove(MemoryUsage.sizeOfDoubleArray(weights[weightIndex].length));
            tracker.add(MemoryUsage.sizeOfDoubleArray(newLength));
    ...
    void release() {
        if (storage.length > 0) {
            tracker.remove(MemoryUsage.sizeOfByteArray(storage.length));
            tracker.remove(MemoryUsage.sizeOfDoubleArray(weights.length));
        }
        storage = null;
        weights = null;
        pos = 0;
