...
public class PearsonAlgorithm extends WeightedAlgorithm {
    public PearsonAlgorithm(GraphDatabaseAPI api, ProcedureConfiguration configuration) {
    ...
    private static Double getSimilarityCutoff(ProcedureConfiguration configuration) {
    ...
    public SimilarityComputer<WeightedInput> similarityComputer(final Double skipValue) {
        return skipValue == null ?
                (decoder, s, t, cutoff) -> s.pearson(decoder, cutoff, t, false) :
                (decoder, s, t, cutoff) -> s.pearsonSkip(decoder, cutoff, t, skipValue, false);
    }
