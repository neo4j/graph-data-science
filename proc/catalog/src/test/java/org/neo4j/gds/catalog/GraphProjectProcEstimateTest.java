...
class GraphProjectProcEstimateTest extends BaseProcTest {
    ...
    void computeMemoryEstimationForVirtualGraphWithProperties() {
        ...
        runQueryWithRowConsumer(
            query,
            row -> {
                assertEquals(558648, row.getNumber("bytesMin").longValue());
                assertEquals(558648, row.getNumber("bytesMax").longValue());
            }
        );
    }
    ...
    void computeMemoryEstimationForVirtualGraphWithLargeValues(SoftAssertions softly) {
        ...
        runQueryWithRowConsumer(
            query,
            row -> {
                softly.assertThat(row.getNumber("bytesMin").longValue()).isEqualTo(344128414632L);
                softly.assertThat(row.getNumber("bytesMax").longValue()).isEqualTo(384930603944L);
