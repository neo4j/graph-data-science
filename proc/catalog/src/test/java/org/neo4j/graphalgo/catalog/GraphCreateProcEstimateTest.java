...
class GraphCreateProcEstimateTest extends BaseProcTest {
    ...
    void estimateHeapPercentageForNativeProjection() {
        ...
        runQueryWithRowConsumer(query, map("relProjection", relProjection),
            row -> {
                assertEquals(303496, row.getNumber("bytesMax").longValue());
                assertEquals(303496, row.getNumber("bytesMin").longValue());
    ...
    void virtualEstimateHeapPercentage() {
        ...
        runQueryWithRowConsumer(query, map("relProjection", relProjection),
            row -> {
                assertEquals(30190192, row.getNumber("bytesMin").longValue());
                assertEquals(30190192, row.getNumber("bytesMax").longValue());
                assertEquals(expectedPercentage, row.getNumber("heapPercentageMin").doubleValue());
                assertEquals(expectedPercentage, row.getNumber("heapPercentageMax").doubleValue());
            }
        );
    ...
    void computeMemoryEstimationForNativeProjectionWithProperties() {
        ...
        runQueryWithRowConsumer(query, map("relProjection", relProjection),
            row -> {
                assertEquals(573920, row.getNumber("bytesMin").longValue());
                assertEquals(573920, row.getNumber("bytesMax").longValue());
            }
        );
    }
    ...
    void computeMemoryEstimationForVirtualGraph() {
        ...
        runQueryWithRowConsumer(query,
            row -> {
                assertEquals(303736, row.getNumber("bytesMin").longValue());
                assertEquals(303736, row.getNumber("bytesMax").longValue());
    ...
    void computeMemoryEstimationForVirtualGraphWithProperties() throws Exception {
        ...
        runQueryWithRowConsumer(query,
            row -> {
                assertEquals(574160, row.getNumber("bytesMin").longValue());
                assertEquals(574160, row.getNumber("bytesMax").longValue());
            }
        );
    }
    ...
    void computeMemoryEstimationForVirtualGraphWithLargeValues() {
        ...
        runQueryWithRowConsumer(query,
            row -> {
                assertEquals(170_836_586_784L, row.getNumber("bytesMin").longValue());
                assertEquals(230_841_207_432L, row.getNumber("bytesMax").longValue());
