...
class GraphCreateProcEstimateTest extends BaseProcTest {
    ...
    void estimateHeapPercentageForNativeProjection() {
        ...
        runQueryWithRowConsumer(query, map("relProjection", relProjection),
            row -> {
                assertEquals(295448, row.getNumber("bytesMax").longValue());
                assertEquals(295448, row.getNumber("bytesMin").longValue());
    ...
    void virtualEstimateHeapPercentage() {
        ...
        runQueryWithRowConsumer(query, map("relProjection", relProjection),
            row -> {
                assertEquals(30063136, row.getNumber("bytesMin").longValue());
                assertEquals(30063136, row.getNumber("bytesMax").longValue());
                assertEquals(expectedPercentage, row.getNumber("heapPercentageMin").doubleValue());
                assertEquals(expectedPercentage, row.getNumber("heapPercentageMax").doubleValue());
            }
        );
    ...
    void computeMemoryEstimationForVirtualGraph() {
        ...
        runQueryWithRowConsumer(query,
            row -> {
                assertEquals(295928, row.getNumber("bytesMin").longValue());
                assertEquals(295928, row.getNumber("bytesMax").longValue());
    ...
    void computeMemoryEstimationForVirtualGraphWithProperties() throws Exception {
        ...
        runQueryWithRowConsumer(query,
            row -> {
                assertEquals(558520, row.getNumber("bytesMin").longValue());
                assertEquals(558520, row.getNumber("bytesMax").longValue());
            }
        );
    }
    ...
    void computeMemoryEstimationForVirtualGraphWithLargeValues() {
        ...
        runQueryWithRowConsumer(query,
            row -> {
                assertEquals(170_040_500_648L, row.getNumber("bytesMin").longValue());
                assertEquals(230_045_121_296L, row.getNumber("bytesMax").longValue());
