...
class DfsStreamComputationResultConsumerTest {
    ...
    @Mock
    ComputationResult<DFS, HugeLongArray, DfsStreamConfig> computationResultMock;
    @Mock
    ExecutionContext executionContextMock;
    @Mock
    ProcedureCallContext procedureCallContextMock;
    ...
    void shouldNotComputePath() {
        ...
        var consumer = new DfsStreamComputationResultConsumer();

        var actual = consumer.consume(computationResultMock, executionContextMock);
        assertThat(actual)
            .hasSize(1)
            .containsExactly(
                new DfsStreamResult(0, List.of(1L, 2L), null)
            );
    }
