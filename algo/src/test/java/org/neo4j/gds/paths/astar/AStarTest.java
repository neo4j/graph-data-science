...
class AStarTest {
    ...
    void sourceTarget() {
        ...
        var parameters = new AStarParameters(
            "longitude",
            "latitude",
    ...
    void shouldLogProgress() {
        ...
        var testTracker = TestProgressTracker.create(
            ...
            RelationshipCountProgressTaskFactory.create(AlgorithmLabel.AStar, new Concurrency(4), graph.relationshipCount()),
            new Concurrency(4),
        ...
        AStar.sourceTarget(
            ...
            .compute()
            // We need to consume the result stream in order to close the progress tracker.
            .findFirst();
    ...
    void haversineTest(
        ...
        assertThat(AStar.HaversineHeuristic.distance(sourceLatitude, sourceLongitude, targetLatitude, targetLongitude))
            .isCloseTo(expectedDistance, Offset.offset(1e-2));
    }
}
