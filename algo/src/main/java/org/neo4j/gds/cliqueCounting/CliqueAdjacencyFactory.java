...
public final class CliqueAdjacencyFactory {
    ...
    static CliqueAdjacency createCliqueAdjacency(Graph graph) {
        switch (graph) {
            ...
            case NodeFilteredGraph nodeFilteredGraph -> {
                ...
                var innerAdjacency = createCliqueAdjacency(innerGraph);
                return new NodeFilteredCliqueIntersect(nodeFilteredGraph, innerAdjacency);
            }
            default -> throw new RuntimeException("Unsupported graph type: " + graph.getClass());
        }
