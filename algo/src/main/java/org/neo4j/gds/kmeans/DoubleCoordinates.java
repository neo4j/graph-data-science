...
public class DoubleCoordinates implements  Coordinates{
    ...
    DoubleCoordinates(int k,int dimensions, NodePropertyValues nodePropertyValues){
    ...
    double coordinateAt(int coordinateId, int dimension){
        return  coordinates[coordinateId][dimension];
    }

    @Override
    public void add(int coordinateId, Coordinates outsideCoordinates) {
        if (outsideCoordinates instanceof  DoubleCoordinates doubleCoordinates) {
            for (int dimension = 0; dimension < dimensions; ++dimension) {
                coordinates[coordinateId][dimension] += doubleCoordinates.coordinateAt(coordinateId,dimension);
            }
        }else {
