...
public final class MetricSimilarityComputerFactory {
    ...
    public static MetricSimilarityComputer create(NodeSimilarityParameters parameters){
        ...
        }else if (metric == NodeSimilarityMetric.JACCARD){
            return  new JaccardSimilarityComputer(cutoff);
        }else if (metric == NodeSimilarityMetric.COSINE){
            return new CosineSimilarityComputer(cutoff);
        }
