...
public final class ExitAndAggregation {

    private ExitAndAggregation() {}

    public static ExitAndAggregationParameters create(Graph graph, TraversalParameters parameters) {
        ExitPredicate exitFunction;
        Aggregator aggregatorFunction;
        // target node given; terminate if target is reached
        if (parameters.hasTargetNodes()) {
            List<Long> mappedTargets = parameters.targetNodes().stream()
        ...
        } else if (parameters.hasMaxDepth()) {
        ...
        return new ExitAndAggregationParameters(exitFunction, aggregatorFunction);
    }
}
