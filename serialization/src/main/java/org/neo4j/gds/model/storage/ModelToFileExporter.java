...
public final class ModelToFileExporter {
    ...
    public static void toFile(Model<ModelData, GraphSageTrainConfig> model, Path exportDir) throws IOException {
        // TODO: some validation, blah-blah
        try(var out = new FileOutputStream(exportDir.resolve(model.name()).toFile())) {
            var serializableModel = GraphSageModelSerializer.toSerializable(model);
            serializableModel.writeTo(out);
        }
    }
}
