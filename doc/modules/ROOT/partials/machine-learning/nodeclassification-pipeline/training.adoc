...
The classification model can be xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-predict[applied] to a possibly different graph which classifies nodes.

More precisely, the training proceeds as follows:

. Apply `nodeLabels` and `relationshipType` filters to the graph.
. Apply the node property steps, added according to xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-adding-node-properties[Adding node properties], on the whole graph.
. Select node properties to be used as features, as specified in xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-adding-features[Adding features].
. Split the input graph into two parts: the train graph and the test graph. This is described in xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-configure-splits[Configuring the node splits].
...
The number of folds `k` can be configured as described in xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-configure-splits[Configuring the node splits].
. Each model candidate defined in the xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-configure-model-parameters[parameter space] is trained on each train set and evaluated on the respective validation set for every fold. The evaluation uses the specified primary xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipeline-metrics[metric].
...
| metrics                                                       | List of String    | n/a     | no       | xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipeline-metrics[Metrics] used to evaluate the models.
...
[source, cypher, role=noplay setup-query, group=nc, indent=0]
----
CREATE
...
[source, cypher, role=noplay graph-project-query, group=nc, indent=0]
----
...
| {maxEpochs=100, minEpochs=1, penalty=0.0625, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001} | 0.999999989939394 | 0.9999999912121211 | 0.9999999850000002
|===
