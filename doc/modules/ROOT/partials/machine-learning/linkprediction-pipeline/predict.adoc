...
After xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[training], the runnable model is of type `{modelType}` and resides in the xref::model-catalog/index.adoc[model catalog].
...
We will use the model which we trained in the xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipeline-examples-train-query[train example] which we gave the name `lp-pipeline-model`.
...
We will explain each individually with examples in the xref::machine-learning/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict-examples-mutate[mutate examples] below.
...
We will use the model `lp-pipeline-model`, that we trained in the xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[train example].
