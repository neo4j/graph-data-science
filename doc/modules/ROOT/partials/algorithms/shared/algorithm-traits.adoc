...
[[node-properties]]Node properties:: xref:introduction.adoc#introduction-algorithms-node-properties[Node properties trait]. The algorithm makes use of node properties.