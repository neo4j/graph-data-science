...
| sourceNodes | Node/Number or List or Map | [] | yes | The nodes or node ids or node-bias map to use for computing Personalized Article Rank. To use different bias for different source nodes, use the Map syntax: `{nodeId1: bias1, nodeId2: bias2, ...}`.
