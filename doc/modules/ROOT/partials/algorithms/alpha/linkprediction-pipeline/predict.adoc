...
After xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[training], the runnable model is of type `Link prediction pipeline` and resides in the model catalog.
...
include::partial$/algorithms/alpha/linkprediction-pipeline/specific-predict-configuration.adoc[]
...
We will use the model which we trained in the xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipeline-examples-train-query[train example] which we gave the name `lp-pipeline-model`.
...
We will explain each individually with examples in the xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict-examples-mutate[mutate examples] below.
...
In these examples we will show how to write the predictions to your in-memory graph.
We will use the model `lp-pipeline-model`, that we trained in the xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[train example].
