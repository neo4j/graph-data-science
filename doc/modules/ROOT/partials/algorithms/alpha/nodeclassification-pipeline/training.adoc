...
The train mode, `gds.alpha.ml.pipeline.nodeClassification.train`, is responsible for splitting data, feature extraction, model selection, training and storing a model for future use.
Running this mode results in a classification pipeline of type `Node classification pipeline`, which is then stored in the xref::model-catalog/index.adoc[model catalog].
The classification pipeline can be xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-node-classification-pipelines-predict[applied] to a possibly different graph which classifies nodes.

More precisely, the training proceeds as follows:

. Apply `nodeLabels` and `relationshipType` filters to the graph.
. Apply the node property steps, added according to xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-adding-node-properties[Adding node properties], on the whole graph.
. Select node properties to be used as features, as specified in xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-adding-features[Adding features].
. Split the input graph into two parts: the train graph and the test graph. This is described in xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-configure-splits[Configuring the node splits].
...
The number of folds `k` can be configured as described in xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-configure-splits[Configuring the node splits].
. Each model candidate defined in the xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-configure-model-parameters[parameter space] is trained on each train set and evaluated on the respective validation set for every fold. The training process uses a logistic regression algorithm, and the evaluation uses the specified xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-pipeline-metrics[metric].
...
| metrics                                                       | List of String    | n/a     | no       | xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-pipeline-metrics[Metrics] used to evaluate the models.
...
[source, cypher, role=noplay graph-create-query, group=nc]
----
