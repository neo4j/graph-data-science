...
If these paths are expected to have small length (i.e., a few new nodes) setting a high value for concurrency is discouraged as some of the cores might be left unutilized.
...
[[algorithms-yens-inverse-index]]
=== Graph pruning optimization

On undirected and inverse-indexed graphs, GDS applies a graph pruning optimization in a pre-processing step as described in https://yuede.github.io/files/23_SC_PeeK.pdf[PeeK: A Prune-Centric Approach for K Shortest Path
Computation].

This allows Yen's algorithm to run on a smaller graph, which results in significant performance improvements in most cases. Especially for high values of k.

The graph pruning is applied by default on undirected graphs.

In order to apply on directed graphs, the `indexInverse` parameter has to be used in the relationship projection.
An inverse index allows the algorithm to traverse the relationships of a node according to the opposite orientation.

.The following statement will project a graph using a Cypher projection and store it in the graph catalog under the name 'myIndexedGraph'.
[source, cypher, role=noplay graph-project-query]
----
MATCH (source:Location)-[r:ROAD]->(target:Location)
RETURN gds.graph.project(
...
The following query is identical to the xref:algorithms/yens.adoc#algorithms-yens-examples-stream[stream example].
This time, we execute Yen's algorithm on `myIndexedGraph` which will allow Peek pruning.

[role=query-example, no-result=true]
--
.The following will run the algorithm with PeeK pruning and stream results:
[source, cypher, role=noplay]
----
MATCH (source:Location {name: 'A'}), (target:Location {name: 'F'})
