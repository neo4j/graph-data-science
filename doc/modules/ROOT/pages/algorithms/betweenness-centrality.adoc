...
:image-file: betweenness-centrality.svg
...
.The following statement will project a graph using a Cypher projection and store it in the graph catalog under the name 'myUndirectedGraph'.
[source, cypher, role=noplay graph-project-query]
----
MATCH (source:User)-[r:FOLLOWS]->(target:User)
RETURN gds.graph.project(
