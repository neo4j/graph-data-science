...
If set, `degreeCutoff`  imposes a lower limit on the degree in order for a node to be considered in the comparisons.
if set,  `upperDegreeCutoff` imposes an upper limit on the node degree.
The two parameters can be combined so that only those nodes whose degree falls under a certain segment are considered.
