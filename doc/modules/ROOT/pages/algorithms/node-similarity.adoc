...
= Node Similarity
:description: This section describes the Node Similarity algorithm in the Neo4j Graph Data Science library. The algorithm is based on the Jaccard Similarity score.
:entity: relationship
...
For each pair `n`, `m`, the algorithm computes a similarity for that pair which is the Jaccard similarity of `N(n)` and `N(m)`.
...
A related function for computing Jaccard similarity is described in xref::alpha-algorithms/jaccard.adoc[Jaccard Similarity].
...
[[algorithms-node-similarity-syntax-anonymous]]
=== Anonymous graphs
...
The results are the same as for running write mode with a named graph, see the xref::algorithms/node-similarity.adoc#algorithms-node-similarity-syntax[write mode syntax above].
...
The `write` execution mode extends the `stats` mode with an important side effect: for each pair of nodes we create a relationship with the Jaccard similarity score as a property to the Neo4j database.
...
For example a relationship value of 2 is equal to counting that relationship twice while computing the jaccard similarity.
