...
For more information on relationship orientations, see the xref::graph-create.adoc#relationship-projection-syntax[relationship projection syntax section].
...
The results are the same as for running write mode with a named graph, see the xref::algorithms/degree-centrality.adoc#algorithms-degree-centrality-syntax[write mode syntax above].
