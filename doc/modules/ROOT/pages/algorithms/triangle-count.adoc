...
The algorithm supports node label filtering by allowing you to specify a list of three node labels in the configuration.
The algorithm will then only count triangles where all three nodes have one of the specified labels.


[TIP]
====
Triangle counting is a very expensive operation, especially in large graphs.
To speed up the computation, apart from using xref:algorithms-triangle-count-filtering[node label filtering] if that is applicable to your use-case, you should also consider including only the node labels and relationship types relevant for the computation in the graph projection.
====
