...
See xref:algorithms/scale-properties.adoc#algorithms-scale-properties-examples-list[the list example] for more details.
...
[NOTE]
====
Some of the scalers are required to do divisions as part of their computation.
For example, computing the "Standard Score" requires division by the standard deviation.
...
----
CALL gds.beta.scaleProperties.stream(
  graphName: String,
  configuration: Map
) YIELD
  nodeId: Integer,
...
[source, cypher, role=noplay]
----
CALL gds.beta.scaleProperties.mutate(
  graphName: String,
  configuration: Map
) YIELD
...
[source, cypher, role=noplay]
----
CALL gds.beta.scaleProperties.stats(
  graphName: String,
  configuration: Map
)
YIELD
...
[source, cypher, role=noplay]
----
CALL gds.beta.scaleProperties.write(
  graphName: String,
  configuration: Map
)
YIELD
...
|===
| Name          | Type   | Default | Optional | Description
| type          | String | n/a     | no       | Type of the scaler applied for the properties. Supported values are `MinMax`, `Max`, `Mean`, `Log`, `L1Norm`, `L2Norm`, `Center`, and `StdScore`, case insensitively.
...
[source, cypher, role=noplay]
----
CALL gds.beta.scaleProperties.stream.estimate('myGraph', {
...
[source, cypher, role=noplay]
----
CALL gds.beta.scaleProperties.stream('myGraph', {
  nodeProperties: ['buildYear', 'avgReview'],
  scaler: 'MinMax'
...
In this example we will scale the two hotel properties of `buildYear` and `avgReview` using the xref:algorithms/scale-properties.adoc#algorithms-scale-properties-mean[Mean scaler].
...
[source, cypher, role=noplay]
----
CALL gds.beta.scaleProperties.mutate('myGraph', {
  nodeProperties: ['buildYear', 'avgReview'],
  scaler: 'Mean',
...
[source, cypher, role=noplay]
----
CALL gds.beta.scaleProperties.stats('myGraph', {
  nodeProperties: ['buildYear', 'avgReview'],
  scaler: 'center'
...
[source, cypher, role=noplay]
----
CALL gds.beta.scaleProperties.write('myGraph', {
  nodeProperties: ['buildYear', 'avgReview'],
  scaler: 'stdscore',
