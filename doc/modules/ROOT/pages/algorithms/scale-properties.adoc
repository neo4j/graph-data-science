...
If the lists do not all have the same size, shorter lists will be padded with `NaN` values.
Control this behaviour using the `failOnMisalignedArrays` configuration parameter.
...
It will receive an output value of `NaN` for that property.
This extends to list properties; each missing value in a list property will be replaced with `NaN`.
