...
If the graph contains cycles, the algorithm will still compute the longest path for nodes in components without cycles.
For components with cycles, the algorithm starts from nodes with in-degree 0, and continues exploring paths through nodes that are not part of a cycle.
If in doing so, the algorithm encounters a node belonging to a cycle, it will stop exploring its neighborhood.
Note that such nodes can still be part of the answer since the path from the starting node to them is devoid of cycles.
