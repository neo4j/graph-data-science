...
:entity: graph
:result: The distance of the longeset path for each node
...
----
CALL gds.alpha.longestPath.stream(
  graphName: String,
  configuration: Map
) YIELD
  nodeId: Integer
...
In order to have lumber for the table, the workshop process timber, which takes 1 day to complete. Once the lumber is ready, it is already inthe workshop, therefor it takes zero time to ship it. However, the screws take 3 days to be shipped to the workshop. Only after the workshop has all the requirements met, the table can be constructed, a process that takes 1 day.
...
[source, cypher, role=noplay setup-query]
----
CALL gds.alpha.longestPath.stream("g", {relationshipWeightProperty: "time"})
