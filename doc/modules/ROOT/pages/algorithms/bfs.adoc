...
| sourceNode    | Integer or Node        | The node id of the node where to start the traversal.
...
image::bfs-full-stream-path.svg[Visualization of Breadth First Search stream without early termination conditions,width=300,role=popup]
...
image::bfs-target-nodes-stream-path.svg[Visualization of Breadth First Search stream with target nodes,width=300,role=popup]
...
image::bfs-max-depth-stream-path.svg[Visualization of Breadth First Search stream with max depth,width=300,role=popup]
