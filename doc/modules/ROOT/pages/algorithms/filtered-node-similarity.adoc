:page-aliases: algorithms/alpha/filtered-node-similarity.adoc
...
include::partial$/algorithms/filtered-node-similarity/filtered-node-similarity-specific-algo-config.adoc[]
|===
...
----
MATCH (source:Person|Singer)-[r:LIKES]->(target:Instrument)
