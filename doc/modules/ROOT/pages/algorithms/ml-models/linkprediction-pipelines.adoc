...
The GDS library provides Link prediction, see xref::algorithms/ml-models/linkprediction.adoc[here].
...
* xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-creating-a-pipeline[Creating a pipeline]
* xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-adding-node-properties[Adding node properties]
* xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-adding-features[Adding link features]
* xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-configure-splits[Configuring the relationship splits]
* xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-configure-model-parameters[Configuring the model parameters]
* xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[Training the pipeline]
* xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[Applying a trained model for prediction]
...
include::partial$/algorithms/alpha/linkprediction-pipeline/pipelineInfoResult.adoc[]
--
...
Such steps producing node properties can be chained one after another and created properties can also be used to xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-adding-features[add features].
Moreover, the node property steps that are added to the pipeline will be executed both when xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[training] a model and when the trained model is xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[applied for prediction].
...
The pipeline will now execute the xref::algorithms/fastrp.adoc[fastRP algorithm] in mutate mode both before xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[training] a model, and when the trained model is xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[applied for prediction].
...
Like with node property steps, the feature steps are also executed both at xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[training] and xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[prediction] time.
The supported methods for obtaining features are described xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-supported-features[below].
...
| Name                   | Type    | Description
| pipelineName           | String  | The name of the pipeline.
| featureType            | String  | The featureType determines the method used for computing the link feature. See xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-supported-features[supported types].
...
|===
| Name                  | Type    | Default | Description
| validationFolds       | Integer | 3       | Number of divisions of the training graph used during xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[model selection].
...
We now reconfigured the splitting of the pipeline, which will be applied during xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[training].
--
...
In xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[Training the pipeline], we explain further how the configured model candidates are trained, evaluated and compared.

The allowed model parameters are listed in the table xref::algorithms/ml-models/linkprediction-pipelines.adoc#linkprediction-pipelines-model-configuration-table[Model configuration].
...
Each specified model configuration will be tried out during the model selection in xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-ml-linkprediction-pipelines-train[training].
--

include::partial$/algorithms/alpha/linkprediction-pipeline/training.adoc[]

include::partial$/algorithms/alpha/linkprediction-pipeline/predict.adoc[]
