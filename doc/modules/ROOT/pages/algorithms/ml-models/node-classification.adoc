...
= Node classification
:description: This section describes the Node classification model in the Neo4j Graph Data Science library.
:entity: node
...
Node Classification can be used favorably together with xref::algorithms/ml-models/pre-processing.adoc[pre-processing algorithms].
...
However, a node embedding algorithm could embed the neighborhoods of nodes as a node property, to transfer this information into the Node Classification model (see xref::algorithms/node-embeddings.adoc[Node embeddings]).
...
. Finally, the winning model will be registered in the xref::model-catalog/index.adoc[Model Catalog].
...
include::partial$/algorithms/alpha/nodeclassification/specific-train-configuration.adoc[]
...
include::partial$/algorithms/alpha/nodeclassification/specific-stream-configuration.adoc[]
...
include::partial$/algorithms/alpha/nodeclassification/specific-write-mutate-configuration.adoc[]
...
We will use the model which we trained in the xref::algorithms/ml-models/node-classification.adoc#algorithms-ml-nodeclassification-examples-train-query[train example] which we gave the name `'nc-model'`.
