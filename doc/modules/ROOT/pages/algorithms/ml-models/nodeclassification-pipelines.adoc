...
The GDS library also provides a standalone version of xref::algorithms/ml-models/node-classification.adoc[Node Classification].
...
There are two kinds of pipelines: training pipelines and classification pipelines, both of which reside in the xref::model-catalog/index.adoc[model catalog].
...
One can xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-adding-node-properties[configure] which steps should be included above.
The steps execute GDS algorithms that create new node properties.
After configuring the node property steps, one can xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-adding-features[select] a subset of node properties to be used as features.
The training phase (II) proceeds in a manner akin to the standalone version of xref::algorithms/ml-models/node-classification.adoc[Node Classification], where it can train multiple models, select the best one, and report relevant performance metrics.

After xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-pipelines-train[training the pipeline], a classification pipeline is created.
...
NOTE: xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-node-classification-pipelines-predict[Classification] can only be done with a trained classification pipeline (not with a training pipeline).
...
* xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-creating-a-pipeline[Creating a pipeline]
* xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-adding-node-properties[Adding node properties]
* xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-adding-features[Adding features]
* xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-configure-splits[Configuring the node splits]
* xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-configure-model-parameters[Configuring the model parameters]
* xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-pipelines-train[Training the pipeline]
* xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-node-classification-pipelines-predict[Applying a classification pipeline to make predictions]
...
include::partial$/algorithms/alpha/nodeclassification-pipeline/pipelineInfoResult.adoc[]
--
...
Such steps producing node properties can be chained one after another and created properties can later be used as xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-adding-features[features].
Moreover, the node property steps that are added to the training pipeline will be executed both when xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-pipelines-train[training] a model and when the classification pipeline is xref::algorithms/ml-models/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[applied for classification].
...
For example, xref::algorithms/ml-models/pre-processing.adoc[pre-processing algorithms] can be used as node property steps.
...
Node Classification Pipelines manage splitting the nodes into several sets for training, testing and validating the models defined in the xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-configure-model-parameters[parameter space].
...
. The input graph is split into two parts: the train graph and the test graph. See the xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-configure-splits-train-test-image[example below].
. The train graph is further divided into a number of validation folds, each consisting of a train part and a validation part. See the xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-configure-splits-validation-image[animation below].
...
| validationFolds    | Integer | 3       | Number of divisions of the training graph used during xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-pipelines-train[model selection].
...
We now reconfigured the splitting of the pipeline, which will be applied during xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-pipelines-train[training].
--
...
In xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-pipelines-train[Training the pipeline], we explain further how the configured model candidates are trained, evaluated and compared.

The allowed model parameters are listed in the table xref::algorithms/ml-models/nodeclassification-pipelines.adoc#nodeclassification-pipelines-model-configuration-table[Model configuration].
...
Each specified model configuration will be tried out during the model selection in xref::algorithms/ml-models/nodeclassification-pipelines.adoc#algorithms-ml-nodeclassification-pipelines-train[training].
--

include::partial$/algorithms/alpha/nodeclassification-pipeline/training.adoc[]

include::partial$/algorithms/alpha/nodeclassification-pipeline/predict.adoc[]
