...
= Link prediction
:description: This section describes the Link prediction model in the Neo4j Graph Data Science library. 
:entity: relationship
...
Link Prediction can be used favorably together with xref::algorithms/ml-models/pre-processing.adoc[pre-processing algorithms].

The basic work flow of Link Prediction contains the following parts which are described below:

* xref::algorithms/ml-models/linkprediction.adoc#algorithms-ml-train-test-splitting[Creating training and test graphs]
* xref::algorithms/ml-models/linkprediction.adoc#algorithms-ml-linkprediction-train[Training and Evaluating model candidates]
* xref::algorithms/ml-models/linkprediction.adoc#algorithms-link-prediction-mutate[Applying a model for prediction]
...
5. The winning model will be registered in the xref::model-catalog/index.adoc[Model Catalog].
...
In order to train a Link Prediction model, one needs training and test graphs as described xref::algorithms/ml-models/linkprediction.adoc#algorithms-ml-linkprediction-train[above].
The recommended way to obtain these is by using xref::alpha-algorithms/split-relationships.adoc[`gds.alpha.ml.splitRelationships()`] procedure, once to produce the test graph, and another time for the training graph.
...
xref::algorithms/ml-models/linkprediction.adoc#algorithms-ml-linkprediction-examples-train[Below], is an example usage of how the `splitRelationships` procedure can be used to prepare the required datasets for training.
...
The former is configured by the configuration parameter `negativeSamplingRatio` in xref::alpha-algorithms/split-relationships.adoc[splitRelationships] when using that procedure to generate the test set.
...
include::partial$/algorithms/common-configuration/common-train-configuration-named-graph.adoc[]

include::partial$/algorithms/alpha/linkprediction/specific-train-configuration.adoc[]
...
include::partial$/algorithms/alpha/linkprediction/specific-configuration.adoc[]
...
| xref::common-usage/running-algos.adoc#common-configuration-write-property[writeProperty]         | String      | n/a                    | no       | The relationship property in the Neo4j database to which the result is written.
|===
...
We will use the model which we trained in the xref::algorithms/ml-models/linkprediction.adoc#algorithms-ml-linkprediction-examples-train-query[train example] which we gave the name `'lp-numberOfPosts-model'`.
...
We will again use the model `'lp-numberOfPosts-model'`, as in the xref::algorithms/ml-models/linkprediction.adoc#algorithms-ml-linkprediction-examples-mutate[mutate example].
