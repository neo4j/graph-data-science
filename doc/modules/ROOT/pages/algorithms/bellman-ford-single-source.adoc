...
[role=query-example]
--
.The following will run Bellman-Ford on a component without negative cycles and stream results:
...
[role=query-example]
--
.The following will run Bellman-Ford on a component with a negative cycle and stream results:
[source, cypher, role=noplay]
----
...
[role=query-example]
--
.The following will run Bellman Ford on stats mode and return its statistics.
[source,cypher,role=noplay]
----
MATCH (source:Location {name: 'A'})
...
For this example, we see that the `containsNegativeCycle` field is false.
