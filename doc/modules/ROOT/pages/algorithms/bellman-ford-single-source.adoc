...
Contrary to the xref:algorithms/dijkstra-single-source.adoc[Dijkstra algorithm] which works only for graphs with non-negative edge weights, Bellman-Ford can also handle graphs with negative weights provided that the source cannot reach any node involved in a negative cycle.
A negative cycle is a path starting and ending at the same node with a negative total sum of relationship weights.
