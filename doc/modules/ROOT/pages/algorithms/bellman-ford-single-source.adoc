...
The xref:https://en.wikipedia.org/wiki/Bellman%E2%80%93Ford_algorithm[Bellman-Ford] Shortest Path algorithm  is a path-finding algorithm on weighted graphs.
Given a source node, it attempts to find shortest paths from the source to all of its reachable nodes.

Unlike the xref:algorithms/dijkstra-single-source.adoc[Dijkstra algorithm] which works only on non-negatively weighted graphs, Bellman-Ford can also work for graphs with negative weights on relationships provided that the graph does not contain any negative cycles.
A negative cycle is a path of nodes, which starts and ends at the same node having a negative sum of weights.
When negative cycles exist, shortest paths cannot easily be defined, as it is possible to traverse a negative cycle multiple times to reduce the cost of each path.
In this case, the Bellman-Ford will return a collection of negative cycles.
Note that this collection will not necessarily include all negative cycles in the graph as they can be too many to enumerate.
Instead, there is the constraint that each vertex will be contained in at most one negative cycle.
...
SPFA significantly reduces the computational run time of Bellman-Ford by working only on a subset of the nodes rather than iterating over the set of nodes at each step.
...
[opts="header"]
|===
| index | sourceNodeName | targetNodeName | totalCost | nodeNames       | costs |   route | isNegativeCycle
...
Since the component of  `A` does not contain any negative cycles, the results depict the shortest paths between node `A` and all of its reachable nodes.
...
As can be seen, Bellman-Ford could not yield the shortest paths as it located a negative cycle with -10 cost which it returns.
This is signaled by the `isNegativeCycle` output field which is set to true.
...
Running stats mode can be useful if we want to discover if the graph has any negative cycles, but we do not to have them computed or store them explicitly.
For this example, we can see that the `containsNegativeCycle` field is false as `A` is not in a component with negative cycles.
...
--
After executing the above query, the in-memory graph will be updated with new relationships of type `ROUTE`.
Since `containsNegativeCycle` is true, these relationships represent negative cycles.
The new relationships will store a single property `totalCost`, corresponding to the weight of the negative cycle.
...
Like mutate mode, the default behavior when encountering negative cycles is to not write them back in tne Neo4j database.
