...
The *raw harmonic centrality* for a node _u_ is calculated using the following formula:

image::harmonic-formulas/raw-harmonic.svg[title="Raw harmonic formula for node _u_", float="center"]

That is, for every node _j_ (excluding _u_)  we compute its minimum distance from _u_ and sum up its inverse.
If there is no path  from _u_ to _j_,  _1/d(u,v)_ is treated as `0`.
...
image::harmonic-formulas/norm-harmonic.svg[title="Normalized harmonic formula for node _u_", float="center"]

Here, we divide the raw value by the number of nodes minus one to normalize the returned value.
In this formula, ∞ values are handled cleanly.
The Neo4j GDS Library calculates normalized harmonic centrality values.
