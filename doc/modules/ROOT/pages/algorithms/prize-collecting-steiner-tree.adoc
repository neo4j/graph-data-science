...
Given a graph, a subtree can be formed by selecting a set of relationships under the constraint that the resulting subgraph is connected and there is exactly one path between every two nodes in it.
A graph can have many possible subtrees depending on which nodes/relationships are selected.

Given a weighted graph where each node has a prize, the Prize-Collecting Steiner Tree problem asks for a subtree that  satisfies the following conditions:
...
| nodeId   | Integer | A node in the discovered subtree.
| parentId | Integer | The parent of nodeId in the tree or nodeId if it is equal to the source node.
...
|===
| Name                      | Type    | Description
| effectiveNodeCount        | Integer | The number of nodes in the subtree.
| totalWeight               | Float   | The sum of the weights of the relationships in the  subtree.
| sumOfPrizes               | Float   | The sum of prizes for the nodes in the  subtree.
...
| 6         | 14                | 4089      | 561808      | "[4089 Bytes \... 548 KiB]"
|===
--
