...
To inspect which community each node belongs to we can xref:management-ops/graph-inspection/graph-stream-nodes.adoc#catalog-graph-stream-single-node-property-example[stream node properties].
...
Let us now xref:management-ops/graph-inspection/graph-stream-nodes.adoc#catalog-graph-stream-single-node-property-example[stream node properties] to once again inspect the node community distribution.
