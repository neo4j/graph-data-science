...
The Filtered Node Similarity algorithm will only compute similarity for nodes that have a degree of at least 1. Eve hence shall not be included from the results as her degree is zero.
...
In the following examples we will demonstrate the usage of the Filtered Node Similarity algorithm on this graph. In particular, we will apply the  `sourceNodeFilter`  and `targetNodeFilter`  filters to limit our similarity search to consider only Person nodes having the Singer label in addition.
