...
[[algorithms-kmeans-introduction-sampling]]
...
This is the default sampler for K-Means.

K-Means++::

This sampling strategy adapts the well-known
http://ilpubs.stanford.edu:8090/778/1/2006-13.pdf[K-means++]  initialization algorithm for K-Means.
...
This sampling strategy tries to spread the initial clusters more evenly so as to obtain a better final clustering. This option can be enabled by choosing `kmeanspp` as the  initial sampler in the configuration.
