...
When a training pipeline is xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-train[executed], a classification model is created and stored in the xref::model-catalog/index.adoc[model catalog].
...
One can xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-adding-node-properties[configure] which steps should be included above.
The steps execute GDS algorithms that create new node properties.
After configuring the node property steps, one can xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-adding-features[select] a subset of node properties to be used as features.
...
After xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-train[training the pipeline], a classification model is created.
...
NOTE: xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-predict[Classification] can only be done with a classification model (not with a training pipeline).

The rest of this page is divided as follows:

* xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-creating-a-pipeline[Creating a pipeline]
* xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-adding-node-properties[Adding node properties]
* xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-adding-features[Adding features]
* xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-configure-splits[Configuring the node splits]
* xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-configure-model-parameters[Configuring the model parameter space]
* xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-train[Training the pipeline]
* xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-predict[Applying a classification model to make predictions]
...
include::partial$/machine-learning/nodeclassification-pipeline/pipelineInfoResult.adoc[]
--
...
Such steps producing node properties can be chained one after another and created properties can later be used as xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-adding-features[features].
Moreover, the node property steps that are added to the training pipeline will be executed both when xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-train[training] a model and when the classification pipeline is xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-predict[applied for classification].
...
Node Classification Pipelines manage splitting the nodes into several sets for training, testing and validating the models defined in the xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-configure-model-parameters[parameter space].
...
. The input graph is split into two parts: the train graph and the test graph. See the xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-configure-splits-train-test-image[example below].
. The train graph is further divided into a number of validation folds, each consisting of a train part and a validation part. See the xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-configure-splits-validation-image[animation below].
...
| validationFolds    | Integer | 3       | Number of divisions of the training graph used during xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-train[model selection].
...
We now reconfigured the splitting of the pipeline, which will be applied during xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-train[training].
--

[[nodeclassification-pipelines-configure-model-parameters]]
...
[source, cypher, role=noplay query-example, no-result=true, group=nc, indent=0]
...
Each specified model candidate will be tried out during the model selection in xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-train[training].
...
include::partial$/machine-learning/nodeclassification-pipeline/training.adoc[]

include::partial$/machine-learning/nodeclassification-pipeline/predict.adoc[]
