...
When a training pipeline is xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[executed], a prediction model is created and stored in the xref::model-catalog/index.adoc[model catalog].
...
For the training and test sets, positive examples are xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-configure-splits[selected] from the relationships in the graph.
The negative examples are sampled from non-adjacent nodes.

One can xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-adding-node-properties[configure] which steps should be included above.
The steps execute GDS algorithms that create new node properties.
After configuring the node property steps, one can xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-adding-features[define] how to combine node properties of node pairs into link features.
...
After xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[training the pipeline], a prediction model is created.
...
NOTE: xref::machine-learning/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[Prediction] can only be done with a prediction model (not with a training pipeline).

The rest of this page is divided as follows:

* xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-creating-a-pipeline[Creating a pipeline]
* xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-adding-node-properties[Adding node properties]
* xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-adding-features[Adding link features]
* xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-configure-splits[Configuring the relationship splits]
* xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-configure-model-parameters[Configuring the model parameter space]
* xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[Training the pipeline]
* xref::machine-learning/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[Applying a trained model for prediction]
...
Such steps producing node properties can be chained one after another and created properties can also be used to xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-adding-features[add features].
Moreover, the node property steps that are added to the pipeline will be executed both when xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[training] a pipeline and when the trained model is xref::machine-learning/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[applied for prediction].
...
The pipeline will now execute the xref::machine-learning/node-embeddings/fastrp.adoc[fastRP algorithm] in mutate mode both before xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[training] a model, and when the trained model is xref::machine-learning/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[applied for prediction].
...
Like with node property steps, the feature steps are also executed both at xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[training] and xref::machine-learning/linkprediction-pipelines.adoc#algorithms-link-prediction-pipelines-predict[prediction] time.
The supported methods for obtaining features are described xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-supported-features[below].
...
| Name                   | Type    | Description
| pipelineName           | String  | The name of the pipeline.
| featureType            | String  | The featureType determines the method used for computing the link feature. See xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-supported-features[supported types].
...
|===
| Name                  | Type    | Default | Description
| validationFolds       | Integer | 3       | Number of divisions of the training graph used during xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[model selection].
...
| negativeSamplingRatio | Double  | 1.0     | The desired ratio of negative to positive samples in the test and train set. More details xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-classimbalance[here].
|===
...
We now reconfigured the splitting of the pipeline, which will be applied during xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[training].
--

[[linkprediction-configure-model-parameters]]
...
For information about the available training methods in GDS, logistic regression and random forest, see xref::machine-learning/training-methods.adoc[Training methods].

In xref::machine-learning/nodeclassification-pipelines.adoc#nodeclassification-pipelines-train[Training the pipeline], we explain further how the configured model candidates are trained, evaluated and compared.
...
include::partial$/machine-learning/training-methods/randomForestConfig.adoc[]
...
[source, cypher, role=noplay query-example, no-result=true, group=lp, indent=0]
...
Each specified model candidate will be tried out during the model selection in xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-pipelines-train[training].

[NOTE]
====
These are somewhat naive examples of how to add and configure model candidates.
Please see xref::machine-learning/training-methods.adoc[Training methods] for more information on how to tune the configuration parameters of each method.
====

include::partial$/machine-learning/linkprediction-pipeline/training.adoc[]

include::partial$/machine-learning/linkprediction-pipeline/predict.adoc[]
...
The former is configured by the configuration parameter `negativeSamplingRatio` in xref::machine-learning/linkprediction-pipelines.adoc#linkprediction-configure-splits[configureSplits] when using that procedure to generate the train and test sets.
