...
. Apply `sourceNodeLabel`, `targetNodeLabel`, and `targetRelationshipType` filters to the graph. All subsequent graphs have the same node set.
...
. Apply the node property steps, added according to xref::machine-learning/linkprediction-pipelines/config.adoc#linkprediction-adding-node-properties[Adding node properties], on the graph of the `featureInput` and `contextRelationshipTypes`.
...
| sourceNodeLabel                                               | String            | n/a                  | no       | The name of the node label relationships in the training and test sets should start from footnote:label-filter[This helps to train the model to predict links with a certain label combination.].
| targetNodeLabel                                              | String            | n/a                  | no       | The name of the node label relationships in the training and test sets should end at footnote:label-filter[].
| contextNodeLabels                                            | List ofString      | []                   | yes      | The names of context node labels used to execute the xref::machine-learning/linkprediction-pipelines/config.adoc#linkprediction-adding-node-properties[nodePropertySteps] over. This is in addition to the `sourceNodeLabel` and `targetNodeLabel`.
| contextRelationshipTypes                                      | List of String    | []                   | yes      | The names of context relationship types to use to the xref::machine-learning/linkprediction-pipelines/config.adoc#linkprediction-adding-node-properties[nodePropertySteps] on. This is in addition to the `feature input` set produced from the `targetRelationshipType` (see xref:machine-learning/linkprediction-pipelines/config.adoc#linkprediction-configure-splits[Configuring the relationship splits])
...
| {maxDepth=2147483647, minLeafSize=1, criterion=GINI, minSplitSize=2, numberOfDecisionTrees=10, methodName=RandomForest, numberOfSamplesRatio=1.0} | 0.631481481481481 |  0.725 | 0.666666666666667 | [0.5833333333333334, 0.6944444444444443, 0.5833333333333334, 0.5833333333333334, 0.5833333333333334]
|===
