...
Please note that the `relationshipCount` reported by the xref:management-ops/inspecting-the-graph-catalog.adoc#catalog-graph-list[graph list] procedure is the _directed_ count of relationships summed over all existing relationship types.
