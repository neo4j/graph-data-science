...
The `test` set are highlighted in blue with the negative relationships having dashed lines.
Then if `trainFraction` is 0.6 and `negativeSamplingRatio` 1, it randomly picks 9 * 0.6 = 5 positive relationships plus 1 * 5 = 5 negative relationship as the `train` set.
The `train` set are highlighted in red with the negative relationships having dashed lines.
The remaining 4 relationships in yellow is the `feature-input` set.

.Positive and negative relationships for each set according to the split
image::example-graphs/lp-split-1.png[example graph for LP split,align="center"]

If a nodePropertyStep has `contextNodeLabel` `City` and `contextRelationshipType` `BORN`, then the feature-input graph for the step would be:
