...
* The `test` set contains a `testFraction` fraction of the positive relationships. The remaining positive relationships is referred to as the `testComplement` set. [[def-test-complement]]
...
* If `negativeRelationshipType` is specified, then instead of sampling, all relationships of this type in the graph is partitioned according to the `test` and `train` set size ratio and added as negative relationships.
...
| negativeRelationshipType | String | n/a | The relationshipType to be used as negative relationships added to `test` and `train` sets. It is mutually exclusive parameter with `negativeSamplingRatio`
|===
