...
To find the best links, we need to search through all possible links defined as a pair of nodes.
GDS offers two different search strategies.
...
As an exhaustive every node pair is not feasible with larger graphs, GDS also offers an approximate search.
...
To avoid possibly having to run for a very long time considering all possible new links, we can use the <<algorithms-link-prediction-pipelines-predict-approximate-search, approximate search strategy>>.
