...
We specified `threshold` to filter out predictions with probability less than 50%, and `topN` to further limit output to the top 5 relationships.
...
|===
| relationshipsWritten | samplingStats
| 16                   | {linksConsidered=31, didConverge=true, strategy=approximate, ranIterations=1}
|===
--
