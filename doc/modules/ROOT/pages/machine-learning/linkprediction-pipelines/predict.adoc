...
Relationships marked as context relationships during training are used as such also for computing features in node property steps.
...
| contextNodeLabels                                             | List of String    | from trainConfig           | yes    | The names of context node labels used to execute the xref:machine-learning/linkprediction-pipelines/config.adoc#linkprediction-adding-node-properties[nodePropertySteps] over. This is in addition to the `sourceNodeLabel` and `targetNodeLabel`.
...
| contextRelationshipTypes                                      | List of String    | from trainConfig             | yes      | The names of the relationships used to execute the xref:machine-learning/linkprediction-pipelines/config.adoc#linkprediction-adding-node-properties[nodePropertySteps] over. This is in addition to the `relationshipTypes`.
...
| "Greg"      | "Veselin"   | 0.5
| "Mark"      | "Veselin"   | 0.5
| "Alice"     | "Veselin"   | 0.498775697837398
| "Alice"     | "Will"      | 0.457493656455499
|===

We can see, that our model predicts the most likely links are the five above.
--
...
As we can see in the `samplingStats`, we use the approximate search strategy and check 52 possible links during the prediction.
...
In `Training the pipeline`, we trained another model `lp-pipeline-model-filtered` on `fullGraph` which uses context `City` nodes and context `LIVES` and `BORN` relationships.

We can leverage this model in prediction, optionally overriding node label or relationship type filterings in prediciton.
In this case we do not, and prediction will inherit the filtering from train config of the `lp-pipeline-model-filtering`.
(i.e We predict Person-KNOWS-Person relationships, additionally using `City` nodes and `LIVES` and `BORN` relationships for node properties generation.)
...
| "Greg"      | "Veselin"   | 0.5
| "Mark"      | "Veselin"   | 0.5
| "Alice"     | "Veselin"   | 0.49887285385083
| "Alice"     | "Will"      | 0.457493887854147
|===

We can see that our model predicts the same 5 most likely links with `lp-pipeline-model`. However, the probabilities vary slightly, due to the additional context information used in training and prediction.
--