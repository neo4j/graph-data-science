...
We specified `threshold` to filter out predictions with probability less than 20%, and `topN` to further limit output to the top 5 relationships.

.Results
[opts="header"]
|===
| person1   | person2   | probability
| "Will"   | "Veselin"    | 0.42005719909751
| "Greg"   | "Veselin"    | 0.207924438628989
|===
...
| 0                   | {linksConsidered=16, strategy=exhaustive}
|===
--
...
Although 16 links were considered, we only mutate the best five (since `topN = 5`) that are above our threshold. In this case there is none.
...
|===
| relationshipsWritten | samplingStats
| 16                   | {linksConsidered=46, didConverge=true, strategy=approximate, ranIterations=2}
|===
--
