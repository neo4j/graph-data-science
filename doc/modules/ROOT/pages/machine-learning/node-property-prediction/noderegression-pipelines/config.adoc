...
[source, cypher, role=noplay]
----
CALL gds.alpha.pipeline.nodeRegression.addNodeProperty('pipe', 'beta.scaleProperties', {
...
| "pipe"   | [{name=gds.beta.scaleProperties.mutate, config={scaler=MinMax, contextRelationshipTypes=[], contextNodeLabels=[], mutateProperty=scaledSizes, nodeProperties=sizePerStory}}]
|===
