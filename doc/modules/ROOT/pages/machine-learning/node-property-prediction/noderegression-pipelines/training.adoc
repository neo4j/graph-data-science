...
Node property steps can use additional node labels and relationship types by specifying `contextNodeLabels` and `contextRelationshipTypes` when xref:machine-learning/node-property-prediction/noderegression-pipelines/config.adoc#noderegression-pipelines-adding-node-properties[adding node property steps].

This makes it possible to create richer generated node properties and thereby also richer features.
The way that works is essentially identical to the way it works in xref:machine-learning/node-property-prediction/nodeclassification-pipelines/training.adoc#nodeclassification-pipelines-examples-train-filtering[Node Classification].
