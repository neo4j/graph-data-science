...
To use this property, we can run another algorithm using the `predictedPrice` property, or inspect it using xref::graph-catalog-node-ops.adoc#catalog-graph-examples-stream[`gds.graph.streamNodeProperty`].
