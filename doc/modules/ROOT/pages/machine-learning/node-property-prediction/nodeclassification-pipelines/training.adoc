...
. Apply a filter using the union of `targetNodeLabels` and `contextNodeLabels` and a filter using `relationshipType` to the graph.
. Apply the node property steps, added according to xref:machine-learning/node-property-prediction/nodeclassification-pipelines/config.adoc#nodeclassification-pipelines-adding-node-properties[Adding node properties], on the whole graph.
. Apply `targetNodeLabels` and `relationshipType` filters to the graph.
...
| contextNodeLabels                                             | List of String    | []                   | yes      | Node labels whose nodes should also be included when applying node property steps.
...
=== Training with context filter

In the above example we projected a Person subgraph without relationships and used it for training and testing.
...
Doing so will make available additional nodes and relationships which are not subject to training or prediction during the creation of node properties, which in turn makes it possible to construct richer features.

The following statement will project a graph containing the information about houses and their paintors using a native projection and store it in the graph catalog under the name 'paintingGraph'.
[source, cypher, role=noplay graph-project-query, group=nc]
----
...
We still train a model to predict the class of each house, but use the Painter information instead of node properties to generate new node properties and features that the model uses in training.
We will begin by creating a new pipeline for that purpose running the following queries.
...
[role=query-example,group=nc]
--
.The following will train another model using the same pipeline, adding `contextNodeLabels`:
[source, cypher, role=noplay]
----
...
While this is a toy example, contextual can sometimes provide more valuable information than node properties, and when used together, performance superior to using either alone can be achieved.
