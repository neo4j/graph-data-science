...
* xref::machine-learning/node-property-prediction/nodeclassification-pipelines/config.adoc#nodeclassification-creating-a-pipeline[Creating the pipeline]
* xref::machine-learning/node-property-prediction/nodeclassification-pipelines/config.adoc#nodeclassification-pipelines-adding-node-properties[Adding node properties]
* xref::machine-learning/node-property-prediction/nodeclassification-pipelines/config.adoc#nodeclassification-pipelines-adding-features[Adding features]
* xref::machine-learning/node-property-prediction/nodeclassification-pipelines/config.adoc#nodeclassification-pipelines-configure-splits[Configuring the node splits]
* xref::machine-learning/node-property-prediction/nodeclassification-pipelines/config.adoc#nodeclassification-pipelines-adding-model-candidates[Adding model candidates]
* xref::machine-learning/node-property-prediction/nodeclassification-pipelines/config.adoc#nodeclassification-pipelines-configure-auto-tuning[Configuring the auto-tuning]
...
| {MultiLayerPerceptronClassification=[], RandomForest=[], LogisticRegression=[]}
|===
--
...
For information about the available training methods in GDS, logistic regression, random forest and multi-layer perceptron, see xref::machine-learning/training-methods/index.adoc[Training methods].
...
[.include-with-multi-layer-perceptron]
======
...
| Name            | Type        | Description
| pipelineName    | String      | The name of the pipeline.
| config          | Map         | The multi-layer perceptron config for a potential model. The allowed parameters for a model are defined in the next table.
|===
...
[source, cypher, role=noplay query-example, no-result=true, group=nc]
.The following will add a multi-layer perceptron model with default configuration:
--
...
YIELD parameterSpace
RETURN parameterSpace.RandomForest AS randomForestSpace, parameterSpace.LogisticRegression AS logisticRegressionSpace, parameterSpace.MLP AS MultilayerPerceptronSpace
----
...
| [{maxDepth=2147483647, minLeafSize=1, criterion=GINI, minSplitSize=2, numberOfDecisionTrees=5, methodName=RandomForest, numberOfSamplesRatio=1.0}] | [{maxEpochs=100, minEpochs=1, penalty=0.0, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001, learningRate=0.001}, {maxEpochs=500, minEpochs=1, penalty={range=[1.0E-4, 100.0]}, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001, learningRate=0.001}] | [{maxEpochs=100, minEpochs=1, penalty=0.0, patience=1, methodName=MultilayerPerceptronClassification, batchSize=100, tolerance=0.001, learningRate=0.001}]
|===
--
