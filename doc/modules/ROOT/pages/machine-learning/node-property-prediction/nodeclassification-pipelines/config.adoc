...
For example, an `embedding` property could be created by a node property step, and we expect a `community` property to be already present in the in-memory graph used as input, at train and predict time.
