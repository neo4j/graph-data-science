...
At this time, using Node2Vec as a node property step in a machine learning pipeline (like xref:machine-learning/linkprediction-pipelines/link-prediction.adoc[] and xref:machine-learning/node-property-prediction/index.adoc[]) is _not_ well supported.

Firstly, it is good to know that node property steps (whether Node2Vec or not) are run both by the training pipeline, and by the prediction models that are created from training a pipeline.
...
So since embeddings will not be deterministic between runs, Node2Vec should not be used as a node property step in a pipeline at this time.
