...
Since FastRP is a random algorithm and is xref:machine-learning/node-embeddings/index.adoc#node-embeddings-generalization[inductive] (in other words, capable of generating embeddings for previously unseen nodes) only for `propertyRatio=1.0`, there are some things to have in mind.
