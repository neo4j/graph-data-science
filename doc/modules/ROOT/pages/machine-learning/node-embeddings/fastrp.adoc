...
The FastRP algorithm initially assigns random vectors to all nodes using a technique called _very sparse random projection_ footnote:[Achlioptas, Dimitris. "Database-friendly random projections: Johnson-Lindenstrauss with binary coins." _Journal of computer and System Sciences_ 66, no. 4 (2003): 671-687.].
...
Differently from the original paperfootnote:[Chen, Haochen, Syed Fahad Sultan, Yingtao Tian, Muhao Chen, and Steven Skiena. "Fast and Accurate Network Embeddings via Very Sparse Random Projection." _arXiv preprint arXiv:1908.11512_ (2019).], the GDS implementation splits the concept of iteration weight into the `iterationWeights` and `nodeSelfInfluence` parameters.

Starting with random vectors (_node projections_) and iteratively averaging over node neighborhoods, the algorithm constructs a sequence of _intermediate embeddings_ image:equations/fastrp/fastrp1.svg[e n to the ith] for each node _n_.
More precisely,
...
Since FastRP is a random algorithm and is xref:machine-learning/node-embeddings/index.adoc#node-embeddings-generalization[inductive] (in other words, capable of generating compatible embeddings for previously unseen nodes) only for `propertyRatio=1.0`, there are some things to have in mind.
...
As mentioned in the introduction, iteration weights in the GDS implementation do not include the <<algorithms-embeddings-fastrp-node-self-influence,node self influence>>.
