...
[TIP]
====
FastRP is featured in the end-to-end example Jupyter notebook https://github.com/neo4j/graph-data-science-client/blob/main/examples/fastrp-and-knn.ipynb[FastRP and kNN end-to-end example].
====
...
Since FastRP is a random algorithm and xref:machine-learning/node-embeddings/index.adoc#node-embeddings-generalization[inductive] only for `propertyRatio=1.0`, there are some things to have in mind.
