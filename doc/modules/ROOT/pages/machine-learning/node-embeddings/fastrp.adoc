...
- It supports both undirected and <<algorithms-embeddings-fastrp-node-self-orientation,directed>> graphs.
- It supports weighted graphs via the `relationshipWeightProperty` parameter (see the <<algorithms-embeddings-fastrp-examples-weighted,example>>).
