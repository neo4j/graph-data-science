...
Higher embedding sizes also increase the required memory and compuation time.
Too small embedding sizes can lead to the input features and graph topology only insufficiently encoded in the embedding.


=== Aggregator

An aggregator defines how to combine a nodes embedding and the sampled neighor embeddings from the last layer.
We support the `MEAN` and `POOL` aggregator.

`MEAN` is the less computation and memory intensive but also simpler than `Pool`.


=== Activation function

The activation function is used to convert the input of a neuron in the neural network.
We support leaky `ReLu` and `Sigmoid`.


=== Sample sizes

Each sample size represents a hidden layer with an output of size `Embedding dimension`, which uses the given `Aggregator` and `Activation Function`.
More layers result in more distant neighbors being considered for a nodes' embedding.
...
A sample size means we try to sample at most `n` neighbors from a node.
...
As each epoch the neighbors are resampled, multiple epochs avoid overfitting on specific neighbors.
...
The more batches are sampled, the more accurate the gradient computation.
...
This parameter defines the maximum depth of the RandomWalks to sample positive examples for each node in a batch.

How close similar nodes are depends on your dataset.


=== Negative-sample weight

This parameter defines the weight of the negative samples compared to the positive sample in the loss computation.
Higher values increase the impact of negative samples in the loss and decreases the impact of the positive sample.


=== Penalty L2

This parameter defines the influence of the regularization term to the loss function.
...
An epoch converges if the current and the loss of the previous iteration differ by less than the `tolerance`.
...
| "exampleTrainModel" | true        | 1         | [26.578495437439546]
|===
--
...
|===
| nodeId | embedding
| 0      | [0.5285002575615408, 0.46821818696428763, 0.7081378445260418]
| 1      | [0.5285002575619906, 0.46821818696452133, 0.7081378445255517]
| 2      | [0.5285002575615245, 0.468218186964279, 0.7081378445260598]
| 3      | [0.5285002575601409, 0.4682181869635602, 0.7081378445275678]
| 4      | [0.5285002576044608, 0.4682181869865899, 0.7081378444792635]
| 5      | [0.5285002576668898, 0.4682181870190296, 0.7081378444112223]
| 6      | [0.528500257560335, 0.468218186963661, 0.7081378445273561]
|===
--
