...
----
RETURN gds.ml.oneHotEncoding(['Chinese', 'Indian', 'Italian'], ['Italian']) AS value
----
...
MATCH (p:Person)
RETURN p.name AS name, gds.ml.oneHotEncoding(cuisines, [(p)-[:LIKES]->(cuisine) | cuisine]) AS value
  ORDER BY name
----
