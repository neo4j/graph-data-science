...
Executing Cypher queries on a projected graph can be achieved by leveraging the `gds.cypherdb.create` procedure.
...
Although it is possible to execute arbitrary Cypher queries on the database created by the `gds.cypherdb.create` procedure, not every aspect of Cypher is implemented yet.
...
If we need to drop the GDS database earlier, there are 2 ways to achieve this:
 ...
 2. Using the <<drop-cypher-db-procedure-syntax, `gds.cypherdb.drop`>> procedure
