...
| writeConcurrency       | Integer               | 'concurrency'     | yes | The number of concurrent threads used for writing the node properties.
|===
...
|===
| Name                   | Type                  | Default           | Optional | Description
| nodeFilter             | String                | n/a               | no       | A Cypher predicate for filtering nodes in the input graph. See xref:management-ops/graph-creation/graph-filter.adoc[Projecting a subgraph].
include::partial$/algorithms/common-configuration/concurrency-entry.adoc[]
|===
