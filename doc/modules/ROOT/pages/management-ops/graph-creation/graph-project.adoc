...
This syntax does not include any node properties.


==== Projecting multiple labels

Use the list syntax to select more than one label for the projection:
...
==== Projecting all the labels

You can use the wildcard operator `*` to select all the labels for the projection.
...
==== Projecting labels with global node properties

You can use the `nodeProperties` configuration parameter to project node properties from nodes that match any of the labels specified in `nodeProjection`:
...
In this example, `prop1` and `prop2` are projected from any node with label `Label 1`, `Label 2`, or `Label 3`.
...
Use the list syntax to select more than one relationship type for the projection:
...
==== Projecting relationship types with global relationship properties

You can use the `relationshipProperties` configuration parameter to project relationship types from relationships that match any of the types specified in `relationshipProjection`:
...
In this example, `prop1` and `prop2` are projected from any relationship with type `TYPE_1`, `TYPE_2`, or `TYPE_3`.
