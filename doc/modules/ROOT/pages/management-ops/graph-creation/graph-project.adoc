...
_Node projections_ and _relationship projections_ describe the way nodes and relationships are respectively loaded (_projected_) from the database into the in-memory graph.
...
Besides the `graphName`, both the `nodeProjection` and `relationshipProjection` are mandatory arguments.
...
A node projection can be specified in three forms:

* As a string (either a node label `<label>` or a wildcard `*`).
* As a list of node labels (`[<label_1>, <label_2>, <label_3>]`).
* As a projection map, where each key is a node label in the projected graph and each value is itself a map.
...
----
<1> The node label in the projected graph.
<2> The node label in the Neo4j graph. If not set, uses the projected label (`<projected_label_1>` here).
<3> The projected node properties for the specified projected label. Can be a string, a list, or a map. An empty map if not specified.
<4> The node property in the projected graph.
<5> The node property in the Neo4j graph. If not set, uses the projected property name (`<projected_prop_1>` here).
<6> The default value if the node property is not defined for a node. If not specified, it takes one of the following values:

 * `Double.NaN` for `Float`
 * `null` for `Float[]`
 * `Integer.MIN_VALUE` for `Integer`
 * `null` for `Integer[]`
...
* The wildcard form does not retain the label information on the projected nodes (see the <<wildcard-example>> for more details).

* All nodes with any of the specified node labels are projected into the GDS graph.
Nodes that have more than one label are projected multiple times.
...
A Relationship projection can be specified in three forms:

* As a string (either a node label `<type>` or a wildcard `*`).
* As a list of node labels (`[<type_1>, <type_2>, <type_3>]`).
* As a projection map, where each key is a relationship type in the projected graph and each value is itself a map.
...
----
<1> The relationship type in the projected graph.
<2> The relationship type in the Neo4j graph. If not set, uses the projected type (`<projected_type_1>` here).
<3> Denotes how Neo4j relationships are represented in the projected graph. Allowed values are `NATURAL` (default), `UNDIRECTED`, `REVERSE`.
<4> Handling of parallel relationships. Allowed values are `NONE` (default), `MIN`, `MAX`, `SUM`, `SINGLE`, `COUNT`.
<5> The projected relationship properties for the specified relationship type. Can be a string, a list, or a map. An empty map if not specified.
<6> The relationship property in the projected graph.
<7> The relationship property in the Neo4j graph. If not set, uses the projected property name (`<projected_prop_1>` here).
<8> The default value if the node property is not defined for a node. If not specified, it defaults to `Double.NaN` for both `Float` and `Integer` types.
...
[[wildcard-example]]
=== Wildcard example
// TODO review text and complete

==== Projecting all the node types
You can also use the wildcard operator `*` to select all the labels for the projection.
However, this does not retain the label information on the projected nodes.
Use the `db.labels()` to retrieve the list of all the labels, and use it as a value for the `nodeProjection` parameter as in the following example:
...
==== Projecting all the relationship types
You can use the wildcard operator `*` to select all the relationship types for the projection.
However, this does not retain the type information on the projected relationships.
Use the `db.relationshipTypes()` to retrieve the list of all the relationship types, and use it as a value for the `relationshipProjection` parameter as in the following example:

----
CALL db.relationshipTypes()
YIELD relationshipType
WITH collect(relationshipType) AS allTypes
CALL gds.graph.project(
  /* graph name */,
  /* node projection */
  allTypes
...
This syntax does not include any relationship properties.