...
Arrow features in GDS are versioned, which allows us to introduce new features without breaking existing ones.
Communication with the server is based on sending commands from the client to the server.
...
Alpha commands are considered deprecated for deletion and will be removed in GDS 2.7.
...
The action type contains the server version, which is currently `V1`.
