...
Random walk with restarts (RWR) samples the graph by taking random walks from a set of start nodes (see the xref:management-ops/graph-creation/sampling/rwr.adoc#algorithms-rwr-syntax-configuration[`startNodes`] parameter below).
On each step of a random walk, there is some probability (see the xref:management-ops/graph-creation/sampling/rwr.adoc#algorithms-rwr-syntax-configuration[`restartProbability`] parameter below) that the walk stops, and a new walk from one of the start nodes starts instead (i.e. the walk restarts).
Each node visited on these walks will be part of the sampled subgraph.
The algorithm stops walking when the requested number of nodes have been visited (see the xref:management-ops/graph-creation/sampling/rwr.adoc#algorithms-rwr-syntax-configuration[`samplingRatio`] parameter below).
...
If the graph is weighted and xref:management-ops/graph-creation/sampling/rwr.adoc#algorithms-rwr-syntax-configuration[`relationshipWeightProperty`]  is specified, the random walks are weighted.
...
To enable such stratification, one can set xref:management-ops/graph-creation/sampling/rwr.adoc#algorithms-rwr-syntax-configuration[`nodeLabelStratification`] to `true` in the algorithm configuration.
...
[[algorithms-rwr-syntax-configuration]]
.Configuration
