...
| memory label:serverless[Aura Graph Analytics Serverless]      | String                | -                   | Declare the memory used for the xref:installation/gds-sessions.adoc[GDS Session] created for the projected graph.
| ttl label:serverless[Aura Graph Analytics Serverless]      | Duration                | PT1H                   | Declare the time to live before the xref:installation/gds-sessions.adoc[GDS Session] created for the projected graph expires due to inactivity.
|===
