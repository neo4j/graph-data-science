...
--
.A new graph can be projected by using the `gds.graph.project.subgraph()` procedure:
...
As an example we can rewrite the xref:management-ops/graph-creation/graph-project-subgraph.adoc#catalog-graph-project-subgraph-node-filter-example[node filter example] from above using parameters instead of integer literals:
