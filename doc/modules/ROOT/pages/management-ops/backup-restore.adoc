...
There are already concepts to circumvent this limitation, such as running algorithms in xref:common-usage/running-algos.adoc#running-algos-write[write mode], xref:management-ops/graph-catalog-export-ops.adoc#catalog-graph-export-csv[exporting graphs to csv] or xref:model-catalog/store.adoc#model-catalog-store-ops[storing models].
