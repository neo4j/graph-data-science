...
Additionally, we will project a few graphs to the graph catalog, for more details see xref:management-ops/graph-creation/graph-project-cypher-projection.adoc[Cypher projections] and xref:management-ops/graph-creation/graph-project.adoc[native projections].
...
.Project `Person` nodes with property `age` and `KNOWS` relationships using Cypher projections:
[source, cypher, role=noplay graph-project-query]
----
MATCH (n:Person)
OPTIONAL MATCH (n)-[r:KNOWS]->(m:Person)
...
| {nodes={Person={}}, relationships={KNOWS={direction="DIRECTED", properties={}}}}
|===
--
