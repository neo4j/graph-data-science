...
NOTE:: We also support `*` as the neo4j-label to load all nodes. However, this does not keep the label information. To retain the label, we recommend using `CALL db.labels() YIELD label YIELD label WITH collect(label) AS allLabels`.
...
NOTE:: We also support `*` as the neo4j-type to load all relationships. However, this does not keep the type information. To retain the type, we recommend using `CALL db.relationshipTypes() YIELD type WITH collect(type) AS allTypes`.
