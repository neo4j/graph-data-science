...
----
CALL gds.graph.drop(
  graphName: String or List of String,
...
| graphName     | String or List of String    | no                   | The name under which the graph is stored in the catalog. To drop multiple graphs at once, they can be passed in a list.
...
| dbName        | String                      | active database name | Then name of the database that was used to project the graph/graphs. When empty, the current database is used.
| username      | String                      | active user          | The name of the user who projected the graph/graphs. Can only be used by GDS administrator.
|===
