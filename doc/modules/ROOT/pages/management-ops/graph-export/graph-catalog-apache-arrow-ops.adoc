...
Export commands are versioned to allow for future changes to the protocol.
Please refer to the xref:management-ops/graph-creation/graph-project-apache-arrow.adoc#arrow-server-versioning[corresponding section] in the xref:management-ops/graph-creation/graph-project-apache-arrow.adoc[] documentation for more details on versioned commands.
