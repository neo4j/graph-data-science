...
The first thing we must do is to project the graph into a GDS Session.
We do that by using a remote Cypher projection, and specifying the `memory` and `ttl` parameters.
...
You can also list the projected graph using the `gds.graph.list()` procedure.
...
We will run xref:algorithms/page-rank.adoc[] and xref:machine-learning/node-embeddings/fastrp.adoc[] in `mutate` mode.
Then we will `stream` the node properties, and lastly we will `write` them back to the AuraDB instance.
...
We will use the mutated `pageRank` property as input to the FastRP algorithm.
...
Now we can stream the node properties back to the AuraDB instance.
...
We can also `write` the results using the `gds.graph.nodeProperty.write()` procedure.
It is of course also possible to `write` results directly as an algorithm execution mode:
...
Before we're done, we will drop the projected graph, which will also delete the GDS Session.
