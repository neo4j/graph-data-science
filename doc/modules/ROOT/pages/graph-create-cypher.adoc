...
Using Cypher projections is a more flexible and expressive approach with diminished focus on performance compared to the xref::graph-create.adoc[native projections].
...
We can see, that the `numberOfPages` are loaded. The default property value is `Double.Nan` and can be changed as in the previous example xref::graph-create-cypher.adoc#node-properties-example[Node properties] by using the Cypher function https://neo4j.com/docs/cypher-manual/current/functions/scalar/#functions-coalesce[_coalesce()_].
...
If we compare the results to the ones from xref::graph-create-cypher.adoc#cypher-relationship-properties[Relationship properties], we can see that using `IS NOT NULL` is filtering out the relationship from Veselin to the book Frankenstein.
This functionality is only expressible with xref::graph-create.adoc[native projections] by creating a xref::create-subgraph.adoc[subraph].
