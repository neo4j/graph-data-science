...
| sessionName label:serverless[Aura Graph Analytics]    | String or Map     | n/a     | no       | The name of the xref:aura-graph-analytics/index.adoc[GDS Session] which holds the model. Accepts the output of the helper function `sessionInfo`.
...
[NOTE]
====
In xref:aura-graph-analytics/index.adoc[Aura Graph Analytics], calling `gds.model.load` is not required -- a stored model is loaded automatically the first time it is used.
Because of this, `gds.model.list` may report a stored model as not loaded until it is used for the first time.
You can still call `gds.model.load` to load a model ahead of time, so that its first use does not incur the loading cost.
====
...
[NOTE]
====
In Aura Graph Analytics, deleting a model removes it from the project-level model catalog.
====
