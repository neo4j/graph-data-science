...
| sessionInfo label:serverless[Aura Graph Analytics]    | String or Map     | n/a     | no       | The name of the xref:aura-graph-analytics/index.adoc[GDS Session] on which the model might exist. Accepts the output of the helper function `sessionInfo`.
