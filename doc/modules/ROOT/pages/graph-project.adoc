...
| nodeProjection        | String, List or Map | no       | One or more xref:graph-project.adoc#node-projection-syntax[node projections].
| relationshipProjection| String, List or Map | no       | One or more xref:graph-project.adoc#relationship-projection-syntax[relationship projections].
...
| nodeProjection         | Map      | The xref:graph-project.adoc#node-projection-syntax[node projections] used to project the graph.
| nodeCount              | Integer  | The number of nodes stored in the projected graph.
| relationshipProjection | Map      | The xref:graph-project.adoc#relationship-projection-syntax[relationship projections] used to project the graph.
...
Thus, the `undirectedKnows` graph contains four relationships, twice as many as the `persons` graph in xref:graph-project-cypher.adoc#graph-project-example-single-label-type[Simple graph].
...
We can see, that the `numberOfPages` property is loaded. The default property value is `Double.NaN` and could be changed using the Map-Syntax the same as for node properties in xref:graph-project-cypher-aggregation.adoc#node-properties-example[Node properties].
...
As mentioned in the xref:graph-project.adoc#graph-project-native-syntax[syntax section], the `validateRelationships` flag controls whether an error will be raised when attempting to project a relationship where either the source or target node is not present in the xref:graph-project.adoc#node-projection-syntax[node projection].
...
We can simulate such a case with the xref:graph-project-cypher.adoc#graph-project-examples[graph present in the Neo4j database]:
