...
[WARNING]
====
Because AGA replaces the Map-based `nodeProjection` and `relationshipProjection` parameters with simplified `nodeLabels` and `relationshipTypes`, per-label or per-type  configurations are not supported. While you can still project properties and configure orientations globally using the `configuration` parameter, use cases requiring advanced per-type aggregation or complex label-specific overrides must use Cypher projections instead.
====
...
| nodePropertyCount          | Integer        | yes      | 0       | The number of node properties per node in the graph.
| relationshipPropertyCount  | Integer        | yes      | 0       | The number of relationship properties per relationship in the graph.
| algorithmCategories        | List of String | yes      | []      | The categories of algorithms you plan to run in the session. Providing this helps produce a more tailored estimate. See the valid values below. Cannot be used together with `algorithms`.
| algorithms                 | Map            | yes      | {}      | The specific algorithms you plan to run in the session, along with their configuration, e.g. `{fastRP: {embeddingDimension: 512}}`. Providing this produces a more tailored estimate than `algorithmCategories`. Cannot be used together with `algorithmCategories`.
|===
