...
[[pipelines-production-tier]]
=== Production tier

.List of all production tier machine learning pipelines operations in the GDS library. Functions are written in _italic_.
[[pipelines-table-beta]]
[role=procedure-listing]
[opts=header,cols="1, 2"]
|===
|Algorithm name | Operation
.1+<.^| xref:machine-learning/linkprediction-pipelines/link-prediction.adoc[Link Prediction Pipeline]
| `gds.pipeline.linkPrediction.addRandomForest`
.1+<.^| xref:machine-learning/node-property-prediction/nodeclassification-pipelines/node-classification.adoc[Node Classification Pipeline]
