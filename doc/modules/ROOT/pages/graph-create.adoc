...
| nodeProjection        | String, List or Map | no       | One or more xref::graph-create.adoc#node-projection-syntax[node projections].
| relationshipProjection| String, List or Map | no       | One or more xref::graph-create.adoc#relationship-projection-syntax[relationship projections].
...
| nodeProjection         | Map      | The xref::graph-create.adoc#node-projection-syntax[node projections] used to project the graph.
| nodeCount              | Integer  | The number of nodes stored in the projected graph.
| relationshipProjection | Map      | The xref::graph-create.adoc#relationship-projection-syntax[relationship projections] used to project the graph.
...
Thus, the `undirectedKnows` graph contains four relationships, twice as many as the `persons` graph in xref::graph-create-cypher.adoc#graph-create-example-single-label-type[Simple graph].
...
We can see, that the `numberOfPages` property is loaded. The default property value is `Double.NaN` and could be changed using the Map-Syntax the same as for node properties in xref::graph-create-cypher.adoc#node-properties-example[Node properties].
...
As mentioned in the xref::graph-create.adoc#graph-create-native-syntax[syntax section], the `validateRelationships` flag controls whether an error will be raised when attempting to create a relationship where either the source or target node is not present in the xref::graph-create.adoc#node-projection-syntax[node projection].
...
We can simulate such a case with the xref::graph-create-cypher.adoc#graph-create-examples[graph present in the Neo4j database]:
