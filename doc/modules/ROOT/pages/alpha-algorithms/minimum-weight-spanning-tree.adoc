...
----
CALL gds.alpha.spanningTree.stream(
  graphName: String,
  configuration: Map
)
YIELD
...
| cost  | Float | The cost of the relationship from parentId to nodeId.
...
----
MATCH (n:Place {id: 'D'})
CALL gds.alpha.spanningTree.stats.estimate('graph', {sourceNode: id(n),relationshipWeightProperty:'cost'})
...
|===
--
[[alpha-algorithms-spanning-tree-examples-sream]]
=== Stream
:!stream-details:
...
----
MATCH (n:Place{id: 'D'})
CALL gds.alpha.spanningTree.stream('graph', {
  sourceNode: id(n),
  relationshipWeightProperty: 'cost'
})
YIELD nodeId,parentId, cost
RETURN gds.util.asNode(nodeId).id AS node, gds.util.asNode(parentId).id AS parent,cost
ORDER BY node
----
