...
[role=query-example]
--
.The following will run the Minimum Weight Spanning Tree algorithm and returns the result in form of statistical and measurement values
[source,cypher,role=noplay]
----
MATCH (n:Place{id: 'D'})
...
[role=query-example]
--
.The following will run the Maximum Weight Spanning tree algorithm and compute its statistics.
[source, cypher, role=noplay]
----
MATCH (n:Place{id: 'D'})
CALL gds.alpha.spanningTree.stats('graph', {
