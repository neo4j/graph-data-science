...
Before Neo4j 5, Cypher did not support filtering `Infinity` values, so GDS offers additional xref:management-ops/utility-functions.adoc[functions] such as  `gds.util.isFinite` function to help filter `Infinity` values from results.
