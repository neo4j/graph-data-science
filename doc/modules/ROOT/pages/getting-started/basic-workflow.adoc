...
This example shows how to create data in Neo4j, project a graph, and run a path finding algorithm both in `stream` and in `write` mode.

:sectnums!:

== Add data to Neo4j

The following Cypher queries create some `:Station` nodes and `:CONNECTION` relationships to model a small train network in the Neo4j database.
Each relationship includes a `distance` numerical attribute representing the distance between two stations.
...
image::quickstart-tube-graph.svg["Tube graph."]

== Project the graph

The following query creates an in-memory graph called `trainGraph` from the `Station` nodes and the `CONNECTION` relationships (including the `distance` relationship property).
