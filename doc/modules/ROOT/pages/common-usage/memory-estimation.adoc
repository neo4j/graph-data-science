...
The xref::graph-create.adoc[`gds.graph.create`] procedures also support `.estimate` to estimate memory usage for just the graph.
...
The `nodeProjection` and `relationshipProjection` parameters follow the same syntax as in xref::graph-create.adoc[`gds.graph.create`].
...
The xref::graph-create-cypher.adoc[`gds.graph.create.cypher`] procedure has to execute both, the `nodeQuery` and `relationshipQuery`, in order to count the number of nodes and relationships of the graph.
