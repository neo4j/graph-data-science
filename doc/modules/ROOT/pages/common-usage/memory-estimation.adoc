...
|===
| Name                                                        | Type                          | Default                | Optional | Description
| xref:graph-project.adoc#node-projection-syntax[node projection]                 | String, List of String or Map | null                   | yes      | The node projection used for anonymous graph creation via a Native projection.
| xref:graph-project.adoc#relationship-projection-syntax[relationship projection] | String, List of String or Map | null                   | yes      | The relationship projection used for anonymous graph creation a Native projection.
...
The xref:graph-project.adoc[`gds.graph.project`] procedures also support `.estimate` to estimate memory usage for just the graph.
...
The `nodeProjection` and `relationshipProjection` parameters follow the same syntax as in xref:graph-project.adoc[`gds.graph.project`].
...
The xref:graph-project-cypher.adoc[`gds.graph.project.cypher`] procedure has to execute both, the `nodeQuery` and `relationshipQuery`, in order to count the number of nodes and relationships of the graph.
