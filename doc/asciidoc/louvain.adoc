...
* <<algorithms-louvain-pre-defined, Seed communities>>
...
.The following will create a sample graph and assign a seed community to the `community` property:
[source, cypher]
----
...
In this example the seed communities are read from the `community` property, and the communities computed by the algorithm are stored to the `newCommunity` property.
...
| seedProperty                         | string  | null                   | yes      | The property name that contains a preliminary seed community (must be a number).
| communityProperty                    | string  | n/a                    | yes      | Deprecated. Replaced by 'seedProperty'.
