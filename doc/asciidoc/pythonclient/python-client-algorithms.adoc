...
In the Python Client, the `stats`, `mutate` and `write` modes have a similar behaviour as described there.
In particular, these modes with return a `dict[str, any]` containing the summary result of running the algorithm.
...
Instead, the result is streamed back to the Python Client, as a `list[dict[str, any]]`.
