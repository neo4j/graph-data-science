...
NOTE: This feature is a lot faster when the <<installation-apache-arrow, Arrow Flight Server>> is enabled.
...
With the <<installation-apache-arrow, Arrow Flight Server> enabled, it is possible to supply more than one data frame, both for nodes and relationships.
...
| drop                    | failIfMissing: Optional[bool] | Series      | <<catalog-graph-drop, Removes the graph>> from the GDS Graph Catalog.
|===
