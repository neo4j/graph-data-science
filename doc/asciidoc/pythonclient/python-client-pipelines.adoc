...
| drop                  | failIfMissing: Optional[bool] | Series                  | <<pipeline-catalog-drop, Removes the pipeline>> from the GDS Pipeline Catalog.
...
This section outlines how to use the Python client to build, configure and train a <<noderegression-pipelines, node regression pipeline>>, as well as how to use the model that training produces for predictions.


=== Pipeline

The creation of the node regression pipeline is very similar to <<noderegression-creating-a-pipeline, how it's done in Cypher>>.
...
| Name                  | Arguments                     | Return type             | Description
| addNodeProperty       | procedure_name: str, +
config: **kwargs        | Series                        | <<noderegression-pipelines-adding-node-properties, Add an algorithm that produces a node property to the pipeline, with optional algorithm-specific configuration>>.
| selectFeatures        | node_properties: +
...
| configureAutoTuning   | config: **kwargs              | Series                  | <<noderegression-pipelines-configure-auto-tuning, Configure the auto-tuning>>.
| train                 | G: Graph, +
config: **kwargs        | NCPredictionPipeline, +
Series          | <<noderegression-pipelines-train, Train the pipeline on the given input graph using given keyword arguments>>.
| node_property_steps   | -                             | DataFrame               | Returns the node property steps of the pipeline.
| feature_properties    | -                             | Series                  | Returns a list of the selected feature properties for the pipeline.
| split_config          | -                             | Series                  | Returns the configuration set up for train-test splitting of the dataset.
| parameter_space       | -                             | Series                  | Returns the model parameter space set up for model selection when training.
...
Another difference is that the `train` Python call takes a <<python-client-graph-object, graph object>> instead of a graph name, and returns a `NRModel` <<python-client-model-object, model object>> that we can run predictions with as well as a pandas `Series` with the metadata from the training.

Please consult the <<noderegression-pipelines, node regression Cypher documentation>> for information about what kind of input the methods expect.
...
In addition to inheriting the methods common to all <<python-client-model-object, model objects>>, node regression models have the following methods:
...
| Name                    | Arguments        | Return type | Description
| predict_mutate          | G: Graph, +
config: **kwargs | Series      | <<noderegression-pipelines-predict-examples-mutate, Predict property values for nodes of the input graph and mutate graph with predictions>>.
| predict_stream          | G: Graph, +
config: **kwargs | DataFrame   | <<noderegression-pipelines-predict-examples-stream, Predict property values for nodes of the input graph and stream the results>>.
| metrics                 | -                | Series      | Returns values for the metrics specified when training, for this particular model.
|===

One can note that the predict methods are indeed very similar to <<noderegression-pipelines-predict, their Cypher counterparts>>.
The three main differences are that:
...
We now continue the <<python-client-pipelines-noderegression-pipe-example, example above>> using the node regression model `trained_pipe_model` we trained there.
Suppose that we have a new graph `H` that we want to run predictions on.
