...
| addNodeProperty       | procedure_name: str, +
                            config: **kwargs       | dict[str, any]                  | <<nodeclassification-pipelines-adding-node-properties, Add an algorithm that produces a node property to the pipeline, with optional algorithm-specific configuration>>.
| selectFeatures        | node_properties: +
                            Union[str, list[str]]  | dict[str, any]                  | <<nodeclassification-pipelines-adding-features, Select node properties to be used as features>>.
| configureSplit        | config: **kwargs         | dict[str, any]                  | <<nodeclassification-pipelines-configure-splits, Configure the train-test dataset split>>.
| addLogisticRegression | parameter_space: +
                            dict[str, any]         | dict[str, any]                  | <<nodeclassification-pipelines-configure-model-parameters, Add a logistic regression model configuration to train as a candidate in the model selection phase>>.
| addRandomForest       | parameter_space: +
                            dict[str, any]         | dict[str, any]                  | <<nodeclassification-pipelines-configure-model-parameters, Add a random forest model configuration to train as a candidate in the model selection phase>>.
| train                 | G: Graph, +
                            config: **kwargs       | NCPredictionPipeline, +
                                                             dict[str, any]          | <<nodeclassification-pipelines-train, Train the pipeline on the given input graph using given keyword arguments>>.
| train_estimate        | G: Graph, +
                            config: **kwargs       | dict[str, any]                  | <<nodeclassification-pipelines-examples-train-memory-estimation, Estimate training the pipeline on the given input graph using given keyword arguments>>.
...
| parameter_space       | -                        | dict[str, list[dict[str, any]]] | Returns the model parameter space set up for model selection when training.
...
[opts="header",cols="8m,6m,7m,11"]
|===
| Name                    | Arguments        | Return type          | Description
| predict_mutate          | G: Graph, +
                            config: **kwargs | dict[str, any]       | <<nodeclassification-pipelines-examples-mutate, Predict classes for nodes of the input graph and mutate graph with predictions>>.
| predict_mutate_estimate | G: Graph, +
                            config: **kwargs | dict[str, any]       | <<nodeclassification-pipelines-examples-predict-memory-estimation, Estimate predicting classes for nodes of the input graph and mutating graph with predictions>>.
| predict_stream          | G: Graph, +
                            config: **kwargs | list[dict[str, any]] | <<node-classification-pipelines-predict-examples-stream, Predict classes for nodes of the input graph and stream the results>>.
| predict_stream_estimate | G: Graph, +
                            config: **kwargs | dict[str, any]       | <<nodeclassification-pipelines-examples-predict-memory-estimation, Estimate predicting classes for nodes of the input graph and streaming the results>>.
| predict_write           | G: Graph, +
                            config: **kwargs | dict[str, any]       | <<nodeclassification-pipelines-examples-write, Predict classes for nodes of the input graph and write results back to the database>>.

| predict_write_estimate  | G: Graph, +
                            config: **kwargs | dict[str, any]       | <<nodeclassification-pipelines-examples-predict-memory-estimation, Estimate predicting classes for nodes of the input graph and writing the results back to the database>>.
...
| Name                  | Arguments              | Return type                     | Description
| addNodeProperty       | procedure_name: str, +
                        config: **kwargs         | dict[str, any]                  | <<linkprediction-adding-node-properties, Add an algorithm that produces a node property to the pipeline, with optional algorithm-specific configuration>>.
| addFeature            | feature_type: str, +
                        config: **kwargs         | dict[str, any]                  | <<linkprediction-adding-features, Add a link feature for model training based on node properties and a feature combiner>>.
| configureSplit        | config: **kwargs       | dict[str, any]                  | <<linkprediction-configure-splits, Configure the feature-train-test dataset split>>.
| addLogisticRegression | parameter_space: +
                          dict[str, any]         | dict[str, any]                  | <<linkprediction-configure-model-parameters, Add a logistic regression model configuration to train as a candidate in the model selection phase>>.
| addRandomForest       | parameter_space: +
                          dict[str, any]         | dict[str, any]                  | <<linkprediction-configure-model-parameters, Add a random forest model configuration to train as a candidate in the model selection phase>>.
| train                 | G: Graph, +
config: **kwargs        | LPPredictionPipeline, +
                                                  dict[str, any]                   | <<linkprediction-pipelines-train, Train the model on the given input graph using given keyword arguments>>.
| train_estimate        | G: Graph, +
                        config: **kwargs         | dict[str, any]                  | <<linkprediction-pipeline-examples-train-estimate, Estimate training the pipeline on the given input graph using given keyword arguments>>.
| node_property_steps   | -                      | list[dict[str, any]]            | Returns the node property steps of the pipeline.
...
| predict_mutate_estimate | G: Graph, +
                            config: **kwargs | dict[str, any]       | <<algorithms-link-prediction-pipelines-predict-examples-estimate, Estimate predicting links between non-neighboring nodes of the input graph and mutating graph with predictions>>.
| predict_stream          | G: Graph, +
...
| predict_stream_estimate | G: Graph, +
                            config: **kwargs | dict[str, any]       | <<algorithms-link-prediction-pipelines-predict-examples-estimate, Estimate predicting links between non-neighboring nodes of the input graph and streaming the results>>.
