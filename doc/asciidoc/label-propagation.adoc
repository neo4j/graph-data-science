...
It is possible to define preliminary labels (identifiers) of nodes using the `labelProperty` parameter.
...
| labelProperty | string  | 'partition'    | yes      | Used to define initial set of labels (must be a number)
