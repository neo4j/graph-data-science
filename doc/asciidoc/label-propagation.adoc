...
At the beginning of the algorithm, every node is initialized with unique label and the labels propagate through the network.

It is possible to define preliminary labels of nodes using the `seedProperty` parameter.
We need to save a preliminary set of labels that we would like to run the Label Propagation algorithm with as node properties.
This property needs to be a number.
...
If there isn't one, it assigns the node a new unique label.
Using this preliminary set of labels, it then sequentially updates each node's label to a new one, which is the most frequent label among its neighbors at every label propagation iteration.
