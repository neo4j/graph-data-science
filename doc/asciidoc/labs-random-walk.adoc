...
The constraints of <<algorithms-pagerank-usage,PageRank>> also apply to Random Walks:
...
CALL gds.alpha.randomWalk.stream(id(home), 3, 1)
YIELD nodeIds
...
CALL algo.randomWalk.stream(start:Object, steps: 100, walks: 10000, {
  graph:'huge',
  nodeQuery:'label or query',
  relationshipQuery:' type or query',
  direction:"IN/OUT/BOTH",
  mode:"node2vec"/"random",
  inOut: 1.0,
  return: 1.0,
  path:false,
  concurrency:4
