...
All nodes from the first node set are compared based on their relationships to the second node set.
...
[NOTE]
====
Running this algorithm requires sufficient memory availability (quadratic growth).
...
[source, cypher]
----
CALL algo.beta.nhs.jaccard(nodeFilter: STRING, relationshipFilter: STRING, {
  write: BOOLEAN,
...
[source, cypher]
----
CALL algo.beta.nhs.jaccard.stream(nodeFilter: STRING, relationshipFilter: STRING, {
  // configuration
})
...
[source, cypher]
----
CALL algo.beta.nhs.jaccard.stream('', 'LIKES', {
  graph: 'huge',
  direction: 'OUTGOING'
})
YIELD node1, node2, similarity
...
[source, cypher]
----
CALL algo.beta.nhs.jaccard('', 'LIKES', {
  graph: 'huge',
  direction: 'OUTGOING',
  write: true
})
...
A value of 0 means that all results are computed and returned in any order.
