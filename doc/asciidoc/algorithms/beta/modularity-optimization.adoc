...
----
CALL gds.beta.modularityOptimization.stream(graphNameOrConfig: String|Map, configuration: Map})
...
| <<common-configuration-write-property,writeProperty>>                            | String  | n/a         | yes      | The property name written back the ID of the partition particular node belongs to.
