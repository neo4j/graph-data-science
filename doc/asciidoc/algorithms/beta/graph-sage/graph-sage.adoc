...
Such algorithms can be <<algorithms-degree-centrality-examples-mutate, Degree Centrality>>, <<algorithms-page-rank-examples-mutate, PageRank>>, <<algorithms-betweenness-centrality-examples-mutate, Betweenness Centrality>>.
...
`gds.degree.mutate` will create node property `degree` for each of the nodes in the in-memory graph, which then can be used as `featureProperty` in the `GraphSAGE.train` mode.
