...
In order to work with multiple labels, we need to project the feature properties into a common feature space of the same dimension.
By specifying `projectedFeatureDimension`, two things happen:
...
Because of this projection, labels with different sets of properties can be used as input for GraphSAGE.
The properties for all nodes are still given in the `featureProperties` configuration parameter.

The projection happens by generating a matrix of random weights per label.
This matrix has the same number of columns as feature properties for that label.
The number of rows is the projected feature dimension.
By multiplying a nodes' values with the weight matrix for its label, we arrive at a feature vector of the shared dimension.
...
CALL gds.beta.graphSage.train(
  'persons',
  {
    ...
    nodePropertyNames: ['age', 'height', 'weight', 'cost'],
    projectedFeatureDimension: 4,
    nodeLabels: ['Person', 'Instrument'],
    relationshipTypes: ['KNOWS', 'LIKES']
  }
)
----

Without using `projectedFeatureDimension`, GraphSAGE can only operate on a monopartite graph.
