...
** <<algorithms-yens-examples-memory-estimation, Memory Estimation>>
...
For `k = 1`, the algorithm behaves exactly like <<algorithms-dijkstra-source-target, Dijkstra's shortest path algorithm>> and returns the shortest path.
...
For the actual path computation, Yen's algorithm uses <<algorithms-dijkstra-source-target, Dijkstra's shortest path algorithm>>.
...
CALL gds.beta.shortestPath.yens.write.estimate('myGraph', {
