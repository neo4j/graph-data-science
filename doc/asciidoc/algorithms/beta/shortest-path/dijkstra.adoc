...
** <<algorithms-dijkstra-examples-stream, Stream>>
** <<algorithms-dijkstra-examples-mutate, Mutate>>
...
[[algorithms-dijkstra-examples-stream]]
=== Stream
...
| 0     | A              | F              | 160       | [0, 2, 3, 4, 5] | [0, 50, 90, 120, 160]
|===
--
...
This can be verified in the <<algorithms-dijkstra-examples, example graph>>.


[[algorithms-dijkstra-examples-mutate]]
=== Mutate
...
After executing the above query the in-memory graph will be updated with a new relationship with type `PATH`.
...
The relationship stores three properties describing the path: `totalCost`, `nodeIds`, `costs`
