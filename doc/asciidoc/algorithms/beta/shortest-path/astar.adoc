...
** <<algorithms-astar-examples-memory-estimation, Memory Estimation>>
...
Unlike <<algorithms-dijkstra-source-target, Dijkstra's shortest path algorithm>>, the next node to search from is not solely picked on the already computed distance.
...
In GDS, the A* algorithm is based on the <<algorithms-dijkstra-source-target, Dijkstra's shortest path algorithm>>.
