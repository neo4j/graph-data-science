...
[abstract]
--
This section describes the K-Nearest Neighbors algorithm in the Neo4j Graph Data Science library.
--

This topic includes:

* <<algorithms-knn-intro, Introduction>>
* <<algorithms-knn-syntax, Syntax>>
** <<algorithms-knn-syntax-anonymous, Anonymous graphs>>
* <<algorithms-knn-examples, Examples>>
** <<algorithms-knn-examples-memory-estimation, Memory Estimation>>
** <<algorithms-knn-examples-stream, Stream>>
** <<algorithms-knn-examples-stats, Stats>>
** <<algorithms-knn-examples-mutate, Mutate>>
** <<algorithms-knn-examples-write, Write>>

[[algorithms-knn-intro]]
== Introduction

The K-Nearest Neighbors algorithm connects each node to a set of neighbors where the distance between two nodes is among the k-th smallest distances from that node to all other nodes.
...
New relationships are created between each node and its k-th nearest new neighbors.
...
A valid sample rate must be in between 0 (exclusive) and 1 (inclusive), the default value is `0.5`.
...
//Furthermore, the neighbors to compare against are taken from the "outgoing" and "incoming" neighbors list.
//"Outgoing" and "incoming" do not refer to the relationships of the node.
//Instead, "outgoing" here means the neighbors of the existing neighbors of a node.
//"Incoming" here means the nodes where a given node is in the neighbors of that other nodes' neighbors.
//MATCH
//  (incoming)-[:NEIGHBOR]->()-[:NEIGHBOR]->(me)-[:NEIGHBOR]->()-[:NEIGHBOR]->(outgoing)
//RETURN
//  me, collect(incoming) as incoming_neighbors, collect(outgoing) as outgoing_neighbors
...
| similarityPairs               | Integer   | The number of neighbors of nodes computed.
...
include::../common-configuration/common-write-relationships-configuration-named-graph.adoc[]
...
include::../common-configuration/common-write-relationships-configuration-anonymous-graph.adoc[]

Please note that the values for `relationshipProjection` or `relationshipQuery` are being used and respected for the graph loading, but loaded relationships are being ignored.

include::knn-specific-algo-config.adoc[]

The results are the same as for running write mode with a named graph, see <<algorithms-knn-syntax, write mode syntax above>>.
...
This graph as persons, each with an age property.
...
In the following examples we will demonstrate using the K-Nearest Neighbors algorithm on this graph.
...
[source, cypher]
----
CALL gds.beta.knn.stream('myGraph', {topK: 1, randomSeed: 42, nodeWeightProperty: 'age'})
...
The TopK is set to 1 to only return the single nearest neighbor for every node.
...
[role=query-example]
--
.The following will run the algorithm, and write back results to the in-mempry graph:
[source, cypher]
----
