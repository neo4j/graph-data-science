...
Another <<algorithms-page-rank-examples-scaler, example>> shows the application of a scaler to normalize the final scores.
...
Comparing the results with the <<algorithms-page-rank-examples-stream, stream example>>, we can see that the relative order of scores is the same.
