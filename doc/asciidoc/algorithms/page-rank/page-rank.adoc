...
In this section we will show examples of executing the PageRank algorithm on a concrete graph.
...
In this e we are using `tolerance: 0.1`, the results are a bit different compared to the ones from <<algorithms-page-rank-examples-stream, stream example>> which is using the default value of `tolerance`.
