...
In the weighted case, the previous score of a node send to its neighbors, is multiplied by the relationship weight und then divided by the sum of the weights of its outgoing relationships.
Negative weights will result in the relationship being ignored.
