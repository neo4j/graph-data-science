...
Node2Vec is a node embedding algorithm that computes a vector representation of a node based on its neighborhood.
...
The target function of the neural network is to predict the likelihood that a node will occur in a walk based on the occurrence of another node.
...
| vector | List<Float> | The computed node embedding.
|===
