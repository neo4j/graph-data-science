...
The Filtered K-Nearest Neighbors algorithm extends our beloved <<algorithms-knn, K-Nearest Neighbors>> algorithm with filtering on source nodes, target nodes or both. You should familiarise yourself with the K-Nearest Neighbors algorithm separately.
...
Just like the K-Nearest Neighbors algorithm is not guaranteed to find _k_ results, the Filtered K-Nearest Neighbors algorithm is not strictly guaranteed to find _k_ results either. But you will increase your odds massively if you employ seeing. In fact, with seeding, the _only_ time you would _not_ get _k_ results is when there literally are not _k_ target nodes in your graph, a very degenerate use case indeed.

Now, the _quality_ of the arbitrary padding results is, well, arbitrary. Something to be aware of.
...
Consider the following graph of five disconnected Person nodes, some of whom are Cool.

image::example-graphs/knn.svg[Visualization of the example graph,align="center"]

[source, cypher, role=noplay setup-query]
----
CREATE (alice:Person:Cool {name: 'Alice', age: 24, lotteryNumbers: [1, 3], embedding: [1.0, 3.0]})
CREATE (bob:Person:Cool {name: 'Bob', age: 73, lotteryNumbers: [1, 2, 3], embedding: [2.1, 1.6]})
CREATE (carol:Person {name: 'Carol', age: 24, lotteryNumbers: [3], embedding: [1.5, 3.1]})
CREATE (dave:Person:Cool {name: 'Dave', age: 48, lotteryNumbers: [2, 4], embedding: [0.6, 0.2]})
CREATE (eve:Person:Cool {name: 'Eve', age: 67, lotteryNumbers: [1, 5], embedding: [1.8, 2.7]});
----
