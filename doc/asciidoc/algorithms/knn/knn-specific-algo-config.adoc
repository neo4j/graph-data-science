...
| nodeProperties                                                   | String or Map or List of String / Maps  | n/a       | no       | The collection of node properties to use for similarity computation along with their selected similarity metrics.
...
If a property is given as string outside a map, the default similarity metric for its type is used.
...
JACCARD, OVERLAP, COSINE, EUCLIDEAN, PEARSON, LONG_PROPERTY_METRIC, DOUBLE_PROPERTY_METRIC.
