...
When a property is a scalar number, the similarity is computed as one divided by one plus the absolute difference between the values:
...
image::knn-formulas/jacard.svg[size of intersection divided by size of union,align="center"]

The Overlap coefficient is defined as follows:

image::knn-formulas/overlap.svg[size of intersection divided by size of minimum set,align="center"]

Both of these metrics give a score in the range `[0, 1]` and no normalization needs to be performed.
The Jaccard similarity is used as the default option for comparing list of integers when the metric is not specified.
...
The default metric used is that of Cosine Similarity, which is defined as follows:

image::knn-formulas/cosine.svg[role="middle"]

Notice that the above formula gives number within the range of `[-1, 1]` .
We normalise the obtained score into the range `[0, 1]` by doing `cosine metric = (cosine similarity + 1)/ 2`.

The other two metrics include the Pearson correlation score and Normalized Euclidean similarity.

The Pearson correlation score is defined as follows:

image::knn-formulas/pearson.svg[role="middle"]

As above, the formula gives numbers in the range `[-1, 1]`, which are normalized into the range `[0, 1]` similarly.

The Normalized Euclidean Similarity works by first calculating the Euclidean distance as follows:

image::knn-formulas/ed.svg[role="middle"]

The result from this formula is a nonnegative value that is not necessarily bounded into the  `[0, 1]` range.
Τo bound the number into this range and obtain a similarity score, we return `similarity= 1/(1+distance)`
...
NB: The validity of this mean is highly context dependent, so take care when applying it to your data domain.
...
----
CREATE (alice:Person {name: 'Alice', age: 24, items:[1,3], vector: [0.1,0.3]})
CREATE (bob:Person {name: 'Bob', age: 73, items: [1,2,3], vector: [0.2,0.6]})
CREATE (carol:Person {name: 'Carol', age: 24, items: [3], vector: [0.1,0.35]})
CREATE (dave:Person {name: 'Dave', age: 48, items: [2,4], vector: [0.6,0.2]})
CREATE (eve:Person {name: 'Eve', age: 67, items: [1,5], vector: [0.8,0.7]});
----
...
If we want calculate similarity based on multiple metrics, we can calculate the similarity for each property individually and take their mean.
As an example, we can use the Normalized Euclidean similarity metric for the vector property and the Overlap metric for the items property.
...
CALL gds.knn.stream('myGraph', {
    topK: 1,
    nodeProperties: [{vector : "EUCLIDEAN"},'age',{items:"OVERLAP"}],
    // The following parameters are set to produce a deterministic result
    randomSeed: 1337,
    concurrency: 1,
    sampleRate: 1.0,
...
|===
| Person1 | Person2 | similarity
| "Alice" | "Carol" | 0.999168744804655
| "Carol" | "Alice" | 0.999168744804655
| "Bob"   | "Carol" | 0.650800310800311
| "Eve"   | "Bob"   | 0.457594716718804
| "Dave"  | "Bob"   | 0.432012432012432
|===
