...
Jaccard similarity is used as the default option for comparing list of integers when the metric is not specified.
...
As an example, we can use the Normalized Euclidean similarity metric for the vector property and the Overlap metric for the items property in addition to the age property.
