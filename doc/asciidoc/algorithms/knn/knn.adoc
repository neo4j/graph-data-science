...
----
CREATE (alice:Person {name: 'Alice', age: 24, items:[1,3], ratings: [1.0,3.0]})
CREATE (bob:Person {name: 'Bob', age: 73, items: [1,2,3], ratings: [2.1,1.6]})
CREATE (carol:Person {name: 'Carol', age: 24, items: [3], ratings: [1.5,3.1]})
CREATE (dave:Person {name: 'Dave', age: 48, items: [2,4], ratings: [0.6,0.2]})
CREATE (eve:Person {name: 'Eve', age: 67, items: [1,5], ratings: [1.8,2.7]});
----
...
As an example, we can use the Normalized Euclidean similarity metric for the ratings property and the Overlap metric for the items property in addition to the age property.
...
CALL gds.knn.stream('myGraph', {
    topK: 1,
    nodeProperties: [{ratings : "EUCLIDEAN"},'age',{items:"OVERLAP"}],
    // The following parameters are set to produce a deterministic result
    randomSeed: 1337,
    concurrency: 1,
    sampleRate: 1.0,
