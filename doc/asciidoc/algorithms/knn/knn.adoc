...
We normalise the cosine similarity score into the range `[0, 1]` by doing `consine metric = (cosine similarity + 1)/ 2`.
...
i.e. the simple mean of the numbers, each of which is in the range `[0, 1]`, giving a total score also in the `[0, 1]` range.
