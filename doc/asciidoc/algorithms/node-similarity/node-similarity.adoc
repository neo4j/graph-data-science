...
The `write` execution mode extends the `stats` mode with an important side effect: for each pair of nodes we create relationship with the Jaccard similarity score as a property to the Neo4j database.
