...
[NOTE]
====
If we would like to compare instead the Instruments to each other in terms of a reverse relationship `HAS_FAN` we would then load the graph with `REVERSE` orientation.
