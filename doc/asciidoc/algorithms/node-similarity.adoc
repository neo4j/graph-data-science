...
Instead of specifying a `writeRelationshipType` and `writeProperty`, we need to specify a `mutateRelationshipType` and `mutateProperty`.
Also, specifying `writeConcurrency` is not allowed in `mutate` mode.
