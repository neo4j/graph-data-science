...
Triangle count implementation includes a `maxDegree` configuration parameter that can be used to exclude nodes from processing if their degree is higher than the configured one.
