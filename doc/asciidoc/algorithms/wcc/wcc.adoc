...
** <<algorithms-wcc-syntax-stream, Stream>>
** <<algorithms-wcc-syntax-stats, Stats>>
** <<algorithms-wcc-syntax-mutate, Mutate>>
** <<algorithms-wcc-syntax-write, Write>>
** <<algorithms-wcc-syntax-anonymous-graphs, Anonymous graphs>>
* <<algorithms-wcc-examples, Examples>>
** <<algorithms-wcc-examples-memory-estimation, Memory Estimation>>
** <<algorithms-wcc-examples-stream, Stream>>
** <<algorithms-wcc-examples-stats, Stats>>
** <<algorithms-wcc-examples-write, Mutate>>
** <<algorithms-wcc-examples-write, Write>>
** <<algorithms-wcc-examples-weighted, Weighted>>
** <<algorithms-wcc-examples-seeding, Seeded components>>
** <<algorithms-wcc-examples-seed-write, Writing Seeded components>>
...
[[algorithms-wcc-syntax-anonymous-graphs]]
=== Anonymous graphs
...
The results are the same as for running write mode with a named graph, <<algorithms-wcc-syntax-write, specified above>>.


[[algorithms-wcc-examples]]
== Examples

In this section we will show examples of executing the Weakly Connected Components algorithm on a concrete graph.
...
[[algorithms-wcc-examples-graph]]
image::wcc-graph.svg[align="center"]
...
This can be verified in the <<algorithms-wcc-examples-graph, example graph>>.

The default behaviour of the algorithm is to run `unweighted`, e.g. without using `relationship` weights.
The `weighted` option will be demonstrated in <<algorithms-wcc-examples-weighted>>

[[algorithms-wcc-examples-stats]]
=== Stats

:stats-syntax: algorithms-wcc-syntax-stats
...
The result shows that `myGraph` has two components and this can be verified by looking at the <<algorithms-wcc-examples-graph, example graph>>.
...
If a relationship does not have the specified weight property, the algorithm falls back to using a default value.
The default fallback value is zero, but can be configured to using the `defaultValue` configuration parameter.
...
This is helpful if we want to retain components from a previous run and is known that no components have been split by removing relationships.
...
. We will create a new in-memory graph that has the result from Step 1 as `nodeProperty`
...
[role=query-example, no-result, group=seeding]
--
...
Note, that we cannot use our already created graph as it does not contain the component id.
...
The result shows that despite not having the `seedProperty` when it was created, the node 'Mats' has been assigned to the same component as the node 'Bridget'.
This is correct because these two nodes are connected.

[[algorithms-wcc-examples-seed-write]]
=== Writing Seeded components

In the <<algorithms-wcc-examples-seeding, previous section>> we demonstrated the `seedProperty` usage in `stream` mode.
It is also available in the other modes of the algorithm.
Below is an example on how to use `seedProperty` in `write` mode.
Note that the example below relies on <<algorithms-wcc-examples-seeding-steps, Steps 1 - 3>> from the previous section.
