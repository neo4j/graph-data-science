...
The Delta-Stepping Shortest Path algorithm computes the shortest path between nodes.
The algorithm supports weighted graphs with positive relationship weights.
The Delta-Stepping Single-Source algorithm computes the shortest paths between a source node and all nodes reachable from that node.
To compute the shortest path between a source and a target node, <<algorithms-delta-source-target, Delta-Stepping Source-Target>> can be used.

The GDS implementation is based on the http://www-m3.ma.tum.de/twiki/pub/MN0506/WebHome/delta.pdf[original description] and uses a binary heap as priority queue.
...
This can be verified in the <<algorithms-delta-single-source-examples, example graph>>.
