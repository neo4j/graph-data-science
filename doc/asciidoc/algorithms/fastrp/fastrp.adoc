...
The embedding stem:[e_n] of node stem:[n], which is to be returned by the algorithm, is initialized as
[stem]
++++
e_n = w_0 \frac{r_n}{\|r_n\|_2},
++++
where stem:[w_0] is the value of `nodeSelfInfluence`, stem:[r_n] is the initial random vector for node stem:[n] and stem:[\|\bullet\|_2] denotes the L2 or https://en.wikipedia.org/wiki/Norm_(mathematics)#Euclidean_norm[euclidean] norm.
We will return to <<algorithms-embeddings-fastrp-node-self-influence,Node Self Influence>> later on.

The algorithm then iteratively constructs _intermediate_ embeddings by averaging either neighboring intermediate embeddings from the previous iteration, or the generated initial random vectors during the first iteration.

At iteration stem:[i], the embedding for node stem:[n] is updated as follows:
[stem]
++++
e_n \leftarrow e_n + w_i \frac{e^i_n}{\|e^i_n\|_2},
++++
where stem:[e^i_n] is the stem:[i]th intermediate embedding for node stem:[n], the values of `iterationWeights` are stem:[[w_1, w_2, \ldots, w_I]].
...
This can be seen as producing non-graphy fallback features when graph structure is (locally) missing.
