...
Most real-world graphs contain node properties which contain important information about the graph and what it represents.
...
The extended FastRP algorithm (FastRPExtended) has the additional configuration parameters parameters `nodePropertyNames` and `propertyDimension`.
The specified node properties are all projected onto vectors of dimensionality `propertyDimension`.
Both the intermediate and final node embeddings will then use `propertyDimension` variables out of the `embeddingDimension` variables for representing node properties and their projections.
In other words, the remaining `embeddingDimension` - `propertyDimension` variables behave as in the original FastRP algorithm.
...
[source, cypher]
----
CALL gds.beta.fastRPExtended.stream.estimate('persons', {embeddingDimension: 128, propertyDimension: 64, nodePropertyNames: ['p1', 'p2']})
YIELD nodeCount, relationshipCount, bytesMin, bytesMax, requiredMemory
----
