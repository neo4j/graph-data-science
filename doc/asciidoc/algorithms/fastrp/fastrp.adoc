...
The extended FastRP algorithm (FastRPExtended) has the additional configuration parameters `featureProperties` and `propertyRatio`.
...
[source, cypher, role=noplay]
----
CALL gds.beta.fastRPExtended.stream.estimate('persons', {embeddingDimension: 128, propertyRatio: 0.5, featureProperties: ['p1', 'p2']})
YIELD nodeCount, relationshipCount, bytesMin, bytesMax, requiredMemory
----
