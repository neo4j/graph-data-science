...
CALL gds.bfs.stream('myGraph', {sourceNode: startNode})
YIELD path
...
CALL gds.bfs.stream('myGraph', {sourceNode: startNode, targetNodes: targetNodes})
YIELD path
UNWIND [ n in nodes(path) | n.tag ] AS tags
RETURN tags
ORDER BY tags
...
MATCH (a:Node{tag:'a'})
WITH id(a) AS startNode
CALL gds.bfs.stream('myGraph', {sourceNode: startNode, maxDepth: 1})
YIELD path
UNWIND [ n in nodes(path) | n.tag ] AS tags
RETURN tags
ORDER BY tags
