...
[abstract]
--
This section describes the Cost Effective Lazy Forward (CELF) Algorithm Influence Maximization algorithm in the Neo4j Graph Data Science library.
--
...
The CELF algorithm is based on a "lazy-forward" optimization. Τhe CELF algorithm dramatically improves the efficiency of the <<algorithms-greedy, Greedy>> algorithm and should be preffered for large networks.
...
[source, cypher]
----
CALL gds.alpha.influenceMaximization.greedy.stream('myGraph', {k: 3, concurrency: 4})
YIELD nodeId, spread
