...
[.include-with-stream]
======
.Run Link Prediction in strean mode on a named graph:
[source, cypher, role=noplay]
----
...
We will use the model which we trained in the <<algorithms-ml-linkprediction-pipelines-train, train example>> which we gave the name `lp-pipeline-mode`.
