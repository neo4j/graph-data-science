...
Although 16 links were considered, only 5 of them had a probability high enough to pass the threshold.
...
Accuracy in this context refers to how close we are in finding the very best actual new possible links according to our models predictions.

The initial set of link predictions for each node is picked at random and verified and then refined in multiple iterations.
...
|===
| relationshipsWritten | samplingStats
| 16                   | {linksConsidered=38, didConverge=true, strategy=approximate, ranIterations=2}
|===
--

As we can see in the `samplingStats`, we use the approximate search strategy and check 38 possible links during the prediction.
