...
| linkFeatureCombiner    | String  | Link feature combiner is used to combine two node feature vectors into the feature vector for the training. Available combiners are `L2`, `HADAMARD` and `COSINE`.
...
| nodeProperties    | List of String    | no      | The names of the node properties that should be used as input features. All property names must exist in the in-memory graph given at train.
|===
