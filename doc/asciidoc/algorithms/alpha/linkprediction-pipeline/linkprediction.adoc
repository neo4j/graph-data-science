...
| "pipe"   | []                | []
           ...
           | {LogisticRegression=[{useBiasFeature=true, maxEpochs=100, minEpochs=1, penalty=0.0, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001}]}
|===
--
...
The `gds.beta.pipeline.linkPrediction.addLogisticRegression` mode is used to set up the train mode with a list of configurations of logistic regression models.
The set of model configurations is called the _parameter space_ which parametrizes a set of model candidates.
The parameter space can be configured by adding multiple logistic regession models.
...
| Name            | Type        | Description
| pipelineName    | String      | The name of the pipeline.
| config          | Map         | The logistic regression config. The allowed parameters for a model are defined in the next table.
|===
...
|===
| parameterSpace
| {LogisticRegression=[{useBiasFeature=true, maxEpochs=100, minEpochs=1, penalty=0.0, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001}, {useBiasFeature=true, maxEpochs=500, minEpochs=1, penalty=0.0, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001}]}
|===
