...
For each node in each potential link, the values of `nodeProperties` are concatenated, in the configured order, into a vector stem:[f].
That is, for each potential link the feature vector for the source node, stem:[s=\[s_1,s_2, \ldots,s_d\]], is combined with the one for the target node,  stem:[t=\[t_1,t_2, \ldots,t_d\]], into a single feature vector stem:[f].
...
| L2                     | stem:[f=\[(s_1 - t_1)^2 , (s_2 - t_2)^2, ..., (s_d - t_d)^2\]]
| HADAMARD               | stem:[f=\[s_1 * t_1, s_2 * t_2, ..., s_d * t_d\]]
| COSINE                 | stem:[f=\frac {\sum_{i=1}^{d}s_{i}t_{i}}{\sqrt {\sum _{i=1}^{d}s_{i}^{2}}\sqrt {\sum _{i=1}^{d}t_{i}^{2}}}]
|===
