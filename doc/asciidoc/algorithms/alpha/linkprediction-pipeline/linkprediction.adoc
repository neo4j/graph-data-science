...
Prior to training one or several model configurations can and should be added to the _parameter space_ with the `gds.beta.pipeline.linkPrediction.addLogisticRegression` and `gds.alpha.pipeline.linkPrediction.addRandomForest` modes.
