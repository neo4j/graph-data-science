...
* <<algorithms-ml-linkprediction-configure-model-parameters, Configuring the model parameter space>>
...
Prior to training one or several model configurations can and should be added to the _parameter space_ with the `gds.beta.pipeline.linkPrediction.addLogisticRegression` and `gds.alpha.pipeline.linkPrediction.addRandomForest` procedures.
For information about the available training methods in GDS, logistic regression and random forest, see <<algorithms-ml-training-methods>>.
...
[[algorithms-ml-linkprediction-logistic-regression-syntax]]
include::../../ml-models/logisticRegressionConfig.adoc[]
...
[[algorithms-ml-linkprediction-random-forest-syntax]]
include::../../ml-models/randomForestConfig.adoc[]
...
| [{maxDepth=2147483647, minSplitSize=2, numberOfDecisionTrees=5, methodName=RandomForest, numberOfSamplesRatio=1.0}] | [{useBiasFeature=true, maxEpochs=100, minEpochs=1, penalty=0.0625, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001}, {useBiasFeature=true, maxEpochs=500, minEpochs=1, penalty=0.0, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001}]
|===
--

The `parameterSpace` in the pipeline now contains the three different model candidates, expanded with the default values.
Each specified model candidate will be tried out during the model selection in <<algorithms-ml-linkprediction-pipelines-train, training>>.

[NOTE]
====
These are somewhat naive examples of how to add and configure model candidates.
Please see <<algorithms-ml-training-methods>> for more information on how to tune the configuration parameters of each method.
====
