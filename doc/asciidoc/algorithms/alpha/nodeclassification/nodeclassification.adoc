...
|===
| Name           | Type          | Description
| classes        | List<Integer> | Sorted list of class ids which are the distinct values of `targetProperty` over the entire graph.
...
| 130858816 | 130890776 | +"[124 MiB ... 124 MiB]"+
|===
--
...
[role=query-example, group=stream]
--
[source, cypher, role=noplay]
----
...
WHERE houseNode:UnkownHouse
RETURN houseNode.color AS classifiedHouse, predictedClass, predictedProbabilities
