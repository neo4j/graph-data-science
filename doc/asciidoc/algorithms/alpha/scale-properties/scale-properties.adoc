...
Scales all property values into the range `[0, 1]` for positive values where the maximum value(s) get the scaled value `1`, according to this formula:

image::equations/scale-properties/max.svg[scaled p equals p minus minimum of p divided by maximum of p minus minimum of p,align="center"]
...
In this example we will scale the two hotel properties of `buildYear` and `avgReview` using the <<algorithms-scale-properties-mean>>.
