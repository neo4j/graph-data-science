...
* <<algorithms-embeddings-random-projection-hyperparameter-tuning, Hyperparameter tuning>>
...
[[algorithms-embeddings-random-projection-hyperparameter-tuning]]
== Hyperparameter tuning

In order to improve the embedding quality using Random Projection on one of your graphs, it is possible to tune several hyperparameters.
We will go through each of them and explain how they work.

For statistically sound results, it is a good idea to reserve a test set excluded from hyperparameter tuning.
...
=== Embedding Dimension
The best value for `embeddingSize` depends on the number of nodes in the graph.
Since the projection becomes more lossy the smaller we set this value, a larger graph may demand a higher embedding dimension.
...
Increasing embedding dimension will however increase memory requirements linearly.
...
=== Sparsity
The `sparsity` parameter maybe be set to any value `>=3`.
Higher values will speed up computation and values up to `sqrt(embeddingSize)` should give comparable results.
