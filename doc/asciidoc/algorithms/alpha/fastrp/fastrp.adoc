...
It iteratively constructs _intermediate_ embeddings by combining neighboring embeddings.
Therefore, each node's embedding depends on a neighborhood of configurable diameter.
...
[[algorithms-embeddings-random-projection-syntax-stream]]
=== Stream mode
...
[[algorithms-embeddings-random-projection-syntax-write]]
=== Write mode
...
The results are the same as for running write mode with a named graph, <<algorithms-embeddings-random-projection-syntax-write, specified above>>.
...
[source, cypher, role=query-example, no-result, group=randomProjection]
----
CALL gds.alpha.randomProjection.stream(
  'persons',
  {
