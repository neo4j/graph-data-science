...
Finding the maximum k-cut for a graph has several applications.
It could for example be used to find communities of entities between which the sum of Bitcoin transfers is maximized.


=== Approximation

Maximum k-cut is a NP-hard problem meaning that the time complexity is exponential for any known algorithm that solves the problem.
Consequently, one has to settle for an approximate solution when solving the problem in practice.
...
The algorithm can be applied to either weighted or unweighted graphs.
In the former case relationships without the specified weight property are given the weight `0.0`.
In the unweighted case the all relationships are treated as weighted with weight `1.0`.

A graph is considered weighted when the `relationshipWeightProperty` config parameter is set to define which relationship property represents weight.
...
The best solution among those derived from all the iterations is then chosen as the final solution.
...
VNS is off (`vnsMaxNeighborhoodOrder = 0`) by default. To use it, experimenting with a maximum order equal to `20` is a good place to start.
...
|===
| Name                   | Type      | Description
| cutCost                | Float     | Sum of the weights of all relationships connecting nodes from different communities.
...
In this section we will show examples of running the Approximate Maximum k-cut algorithm on a concrete graph.
...
We will do this on a small graph of transactions between people.
Using Approximate Maximum k-cut we can find clusters of people between which there are a lot of transactions.
...
To inspect which community each node belongs to we can <<catalog-graph-stream-single-node-property-example, stream node properties>>.
...
Let us now <<catalog-graph-stream-single-node-property-example, stream node properties>> to once again inspect the node community distribution.
...
Comparing this result with that of <<algorithms-approx-max-k-cut-mutate-unweighted-example, unweighted case>> we can see that `Bridget` has moved to another community but the output is otherwise the same.
...
We can see that the result is what we expect, namely the same as in the <<algorithms-approx-max-k-cut-mutate-unweighted-example, mutate unweighted>> example.
