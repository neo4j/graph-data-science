...
NOTE: The names specified in `nodePropertyNames` configuration parameter need to be exist into the in-memory graph.

[source, cypher, role=query-example, no-result=true, test-group=graphSage]
----
CALL gds.alpha.graphSage.train(
  'persons',
  {
...
Once we have a trained model (named `graphSage`) we can use it to generate and stream the embeddings
...
Below is an example of how to generate node embeddings and store them in Neo4j.
