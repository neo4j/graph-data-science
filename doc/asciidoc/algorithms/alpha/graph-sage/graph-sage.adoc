...
GraphSAGE is an _inductive_ framework making use of node feature information to generate node embeddings.
...
[[algorithms-embeddings-graph-sage-syntax-stream]]
=== Stream mode
...
| embedding | List<Double> | The computed node embedding.
|===


[[algorithms-embeddings-graph-sage-syntax-write]]
=== Write mode
...
|===
| Name               | Type                 | Description
| startLoss          | Double               | The Loss value before the first epoch.
| epochLosses        | Map<String, Double>  | A map containing the loss for each of the ran epochs, may be less than the configured epochs.
...
The results are the same as for running write mode with a named graph, <<algorithms-embeddings-graph-sage-syntax-write, specified above>>.
