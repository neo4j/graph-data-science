...
:modelType: Node classification model
:entity: node
...
After <<algorithms-ml-nodeclassification-pipelines-train, training>>, the runnable model is of type `Node classification model` and resides in the <<model-catalog-ops, model catalog>>.
