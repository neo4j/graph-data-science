...
Running this mode results in a classification pipeline of type `Node classification pipeline`, which is then stored in the <<model-catalog-ops, model catalog>>.
The classification pipeline can be <<algorithms-node-classification-pipelines-predict, applied>> to a possibly different graph which classifies nodes.
...
Here we can observe that the model candidate with penalty `0.0625` performed the best in the training phase, with an `F1_WEIGHTED` score nearing 1 over the train graph. On the test graph, the model scores a bit lower at about 64%.
//TODO correct scores in this paragraph
