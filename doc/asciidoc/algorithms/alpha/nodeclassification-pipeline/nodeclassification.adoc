...
* <<algorithms-ml-nodeclassification-configure-model-parameters, Configuring the model parameter space>>
...
Prior to training one or several model configurations can and should be added to the _parameter space_ with the `gds.beta.pipeline.nodeClassification.addLogisticRegression` and `gds.alpha.pipeline.nodeClassification.addRandomForest` procedures.
...
Each specified model candidate will be tried out during the model selection in <<algorithms-ml-nodeclassification-pipelines-train, training>>.
