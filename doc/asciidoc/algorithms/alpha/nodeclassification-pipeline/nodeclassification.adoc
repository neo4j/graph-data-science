...
The property representing the class of the node is referred to as the target property.
GDS supports both binary and multi-class node classification.

In GDS, we have Node Classification pipelines which offer an end-to-end workflow, from features extraction to node classification.
...
The classification pipeline can be used to predict the value of the `target` property (class) of previously unseen nodes.
...
[source, cypher, role=noplay]
----
CALL gds.alpha.ml.pipeline.nodeClassification.addNodeProperty('pipe', 'alpha.scaleProperties', {
