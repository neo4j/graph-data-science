...
Prior to training one or several model configurations can and should be added to the _parameter space_ with the `gds.beta.pipeline.nodeClassification.addLogisticRegression` and `gds.alpha.pipeline.nodeClassification.addRandomForest` modes.
...
include::logisticRegressionConfig.adoc[]
...
include::randomForestConfig.adoc[]
...
|===
| parameterSpace
| {RandomForest=[], LogisticRegression=[{maxEpochs=100, minEpochs=1, penalty=0.0625, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001}, {maxEpochs=500, minEpochs=1, penalty=0.0, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001}]}
|===
