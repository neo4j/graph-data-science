...
[role=query-example,group=nc]
--
.The following will add a feature to the pipeline. Here we assume that the input graph contains a property `sizePerStory` and `scaledSizes` was created in a `nodePropertyStep`.
[source, cypher, role=noplay]
----
CALL gds.alpha.ml.pipeline.nodeClassification.addFeatures('pipe', ['scaledSizes', 'sizePerStory'])
YIELD name, featureSteps
----
