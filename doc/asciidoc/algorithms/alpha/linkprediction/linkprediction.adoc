...
Thus, for each relationship we have one feature vector for the source node, stem:[s=\[s_1,s_2, \ldots,s_d\]], and one for the target node,  stem:[t=\[t_1,t_2, \ldots,t_d\]].
Thereafter, the algorithm uses a _link feature combiner_ to combine the two node feature vectors into a single feature vector stem:[f] for the training example.
There are three supported link feature combiners:

* `L2`
** which gives a feature vector stem:[f=\[(s_1 - t_1)^2 , (s_2 - t_2)^2, ..., (s_d - t_d)^2\]].
* `HADAMARD`
** which gives a feature vector stem:[f=\[s_1 * t_1, s_2 * t_2, ..., s_d * t_d\]].
* `COSINE`
** which gives a single scalar feature, using the the link:http://en.wikipedia.org/wiki/Cosine_similarity[Cosine similarity] between stem:[s] and stem:[t] given by stem:[f=
\frac {\sum_{i=1}^{d}s_{i}t_{i}}{\sqrt {\sum _{i=1}^{d}s_{i}^{2}}\sqrt {\sum _{i=1}^{d}t_{i}^{2}}}.
]
