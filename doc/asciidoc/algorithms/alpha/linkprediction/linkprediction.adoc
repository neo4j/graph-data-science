...
To limit the size of output, configurations for two parameters are required; `topN` retains the most probable predictions and `threshold` retains predictions whose probability is above the threshold.
...
The AUCPR metric can be adjusted to either evaluate in a _traditional_ way or to compensate for class imbalance for more interpretable average precision measurement.
It is common for reasons of resource limitations to down sample the majority class of non-connected pairs and to perform training and evaluation on (nearly) balanced data.

The configuration parameter `classRatio` determines the weighting of negative example (non-connected node pairs) to connected node pairs.
This means upweighting the false positives by this ratio when computing precision.
Setting `classRatio` to `1.0` is consistent with the traditional evaluation method and is recommended for comparison with other models evaluated this way.

The AUCPR, or average precision, is then however overly optimistic because in reality there are much more negative examples which have been sampled out.
There are `n(n-1)/2` node pairs in the graph, which can be used to compute the _true_ class ratio as the ratio of `nodePairs - relationshipCount` to `relationshipCount`.
By setting `classRatio` to a higher value than `1.0`, or even to the true class ratio, the average precision is more accurate.
Since the evaluation is typically done on a graphs created by the `splitRelationships` procedure, this entails a set of equal number of positive and negative examples.
...
The `metrics` map contains for each metric description (currently only `AUCPR`), the corresponding results for that metric.
