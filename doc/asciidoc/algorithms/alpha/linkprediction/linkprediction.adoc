...
In our procedure, the AUCPR metric is used which is considered more suitable than the commonly used AUROC metric for imbalanced data.
One can also specify the configuration parameter `classRatio` which determines the weighting of negative example (non-connected node pairs) to connected node pairs.
This means upweighting the false positives by this ratio when computing precision.

Note that the `classRatio` can be set arbitrarily, and not necessarily as the _true_ class ratio it the graph.
There are `n(n-1)/2` node pairs in the graph, which can be used to compute the true class ratio as the ratio of `nodePairs - relationshipCount` to `relationshipCount`.
Since the evaluation is typically done on graphs created by the `splitRelationships` procedure, this entails a set of equal number of positive and negative examples.
...
To compensate for the under sampling of negative examples, a higher value than `1.0`, or even the _true_ class ratio, can be used.
