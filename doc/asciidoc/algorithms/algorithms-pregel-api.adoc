...
For an example on how to expose a custom Pregel computation via a Neo4j procedure, have a look at the https://github.com/neo4j/graph-data-science/tree/master/examples/pregel-example/src/main/java/org/neo4j/graphalgo/beta/pregel[Pregel examples].
...
This can be achieved by overriding the `org.neo4j.graphalgo.beta.pregel.PregelComputation.masterCompute` function of the `PregelComputation`.
...
The `masterCompute` function has access to the `org.neo4j.graphalgo.beta.pregel.PregelContext.MasterComputeContext`.
...
| <<common-configuration-relationship-weight-property,relationshipWeightProperty>> | String    | null          | If set, the values stored at the given property are used as relationship weights during the computation. If not set, the graph is considered unweighted.
...
In addition, the config parameter of the custom computation must be a subtype of `org.neo4j.graphalgo.beta.pregel.PregelProcedureConfig`.
