...
** <<algorithms-pregel-api-master-compute, Master Computation >>
...
Some Pregel programs may require logic that is executed after all threads have finished the current superstep, for example to reset or evaluate a global datastructure.
This can be achieved by overwriting the `org.neo4j.graphalgo.beta.pregel.PregelComputation.masterCompute` function of the `PregelComputation`.
This function will be called by a single thread after each compute thread has finished its work of the current superstep.

The `masterCompute` function has access to the `org.neo4j.graphalgo.beta.pregel.PregelContext.MasterComputeContext`.
This context is similar to the `ComputeContext` but is not tied to a specific node and does not allow the sending of messages.
