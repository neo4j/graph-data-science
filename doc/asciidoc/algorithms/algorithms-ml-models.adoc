...
The parameters `maxIterations`, `tolerance` and `patience` control for how long the training will run until termination.
These parameters give ways to limit a computational budget. In general, higher `maxIterations` and `patience` and lower `tolerance` lead to longer training but higher quality models.
...
More precisely, `maxIterations` is the maximum number of epochs trained until termination.
...
It is also possible, via `minIterations`, to control a minimum number of epochs before the above termination criteria enter into play.
