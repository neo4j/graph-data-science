...
Additionally by specifying a node weight via the `nodeWeightProperty` key, we can control the influence of a nodes label onto its neighbors.
During the computation of the weight of a specific label, the node property will be multiplied by the weight of that nodes relationships.
