...
If the estimation should be based on an existing graph, the `graphNameOrConfig` should be set to the name of that graph.
Then the memory of the graph is estimated using the original configuration for creating the existing graph.
Otherwise, the graph estimation is based on the configuration pertaining to anonymous loading or so-called fictive estimation controlled by the options in the table below.
For the anonymous case, configuration is used as for <<native-projection>> or <<cypher-projection>>.
