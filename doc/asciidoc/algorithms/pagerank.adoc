...
The above query returned a so called centrality histogram which can be used to monitor the distribution of page rank values across all computed nodes.
This can be useful for inspecting the computed scores or perform normalizations.
