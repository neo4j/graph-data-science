...
| modularities         | Integer[] | The final modularity scores for each level.
...
| communities | Integer[] | Community IDs for each level. `Null` if `includeIntermediateCommunities` is set to false.
|===
