...
YIELD nodePropertiesWritten, communityCount, modularity, createMillis, computeMillis, writeMillis
----
...
| nodePropertiesWritten | Integer   | The number of nodes considered.
...
| communityIds | Integer[] | Community IDs for each level. `Null` if `includeIntermediateCommunities` is set to false.
|===
