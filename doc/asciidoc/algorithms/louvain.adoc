...
| levels               | INTEGER         | The number of supersteps the algorithm actually ran.
| modularity           | FLOAT           | The final modularity score.
| modularities         | LIST of INTEGER | The final modularity scores for each level.
...
| seedProperty                   | STRING   | n/a     | yes      | Used to set the initial community for a node. The property value needs to be a number.
| levels                         | INTEGER  | 10      | yes      | The maximum number of levels in which the graph is clustered and then condensed.
| innerIterations                | INTEGER  | 10      | yes      | The maximum number of iterations that the modularity optimization will run for each level.
| tolerance                      | FLOAT    | 0.0001  | yes      | Minimum change in modularity between iterations. If the modularity changes less than the tolerance value, the result is considered stable and the algorithm returns.
| includeIntermediateCommunities | BOOLEAN  | false   | yes      | Indicates whether to write intermediate communities. If set to false, only the final community is persisted.
|===
...
| community   | INTEGER         | The community ID of the final level.
| communities | LIST of INTEGER | Community IDs for each level. `Null` if `includeIntermediateCommunities` is set to false.
|===
