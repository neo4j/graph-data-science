...
We can load named graphs using either a <<label-relationship-type-projection, label and relationship-type>> or a <<cypher-projection, Cypher>> projection.
...
The in-memory graph models offer different ways of handling multiple (parallel) relationships between a given pair of nodes.
...
As for the Huge graph, we can specify a `deduplicateRelationships` strategy (see <<deduplication-of-parallel-relationships>>).
...
Optionally, we can define an aggregation function to set the deduplication behavior and a default property value which is used for absent property values (see <<deduplication-of-parallel-relationships>>).
