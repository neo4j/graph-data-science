...
[[page-rank-memory-requirements-sample-graphs]]
== Memory Requirements on Sample Graphs

We can use the <<algo-memrec-procedure>> to compute the memory requirements of graph algorithms.

.The following returns the memory requirements for running PageRank on the Heavy Graph:
[source,cypher]
----
CALL algo.memrec(null, null, "pageRank", {graph: "huge", concurrency: 8})
...
[source,cypher]
----
CALL algo.memrec(null, null, "pageRank", {graph: "heavy", concurrency: 8})
YIELD mapView
