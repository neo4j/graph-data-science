...
We want to use the kNN algorithm to identify similar customers and base our product recommendations on that.
In order to be able to leverage topological information about the graph in kNN, we will first create node embeddings using FastRP.
...
Note that we use the algorithm's `write` mode to write the properties back to our database, so that we can analyze them later using Cypher.
