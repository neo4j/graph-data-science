...
To exemplify listing pipelines, we create a <<algorithms-ml-nodeclassification-pipelines, node classification pipeline>> and a <<algorithms-ml-linkprediction-pipelines, link prediction pipeline>> so that we have something to list.

.Creating a link prediction training pipelines:
[source, cypher, role=noplay setup-query]
----
CALL gds.alpha.ml.pipeline.linkPrediction.create('lpPipe')
----

.Creating node classification training pipelines:
[source, cypher, role=noplay setup-query]
----
CALL gds.alpha.ml.pipeline.nodeClassification.create('ncPipe')
----
