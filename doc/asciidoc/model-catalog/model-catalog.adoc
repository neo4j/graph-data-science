...
[.beta-symbol]
[.tier-note]
The model catalog is in the beta tier.
...
* `stored`: whether the model is <<catalog-model-store,stored>>
* `loaded`: whether the model is <<catalog-model-load,loaded>>
...
** can also contain algorithm specific model details.
* `configuration`: the configuration used for training the model.
* `stored`: whether the model was <<catalog-model-store,stored>>
* `loaded`: whether the model was <<catalog-model-load,loaded>>
* `creationTime`: the time at which the model was registered in the catalog.
