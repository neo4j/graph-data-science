...
| failIfMissing | Boolean   | true    | yes      | By default, the library will raise an error when trying to remove a non-existing model. When set to `false`, the procedure returns empty result.
|===
