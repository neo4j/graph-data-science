...
[source, cypher, role=noplay]
----
CALL gds.beta.pipeline.linkPrediction.addLogisticRegression('pipe', {maxEpochs: 500, penalty: {range: [1e-4, 1e4]}})
YIELD parameterSpace
...
We now reconfigured the auto-tuning to use 100 trials during <<linkprediction-pipelines-train, training>>.
--
