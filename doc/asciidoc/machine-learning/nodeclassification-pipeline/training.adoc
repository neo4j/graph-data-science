...
. Each model candidate defined in the <<nodeclassification-pipelines-configure-model-parameters,parameter space>> is trained on each train set and evaluated on the respective validation set for every fold. The evaluation uses the specified primary <<nodeclassification-pipeline-metrics,metric>>.
