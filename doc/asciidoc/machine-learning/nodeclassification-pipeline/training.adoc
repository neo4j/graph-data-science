...
. Each model candidate defined in the <<nodeclassification-pipelines-configure-model-parameters,parameter space>> is trained on each train set and evaluated on the respective validation set for every fold. The training process uses a logistic regression algorithm, and the evaluation uses the specified <<nodeclassification-pipeline-metrics,metric>>.
...
| {maxEpochs=100, minEpochs=1, penalty=0.0625, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001, learningRate=0.001} | 0.999999989939394 | 0.9999999912121211 | 0.9999999850000002
|===
