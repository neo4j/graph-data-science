...
| modelInfo     | Map      | Detailed information about the trained model. Always includes the `modelName` and `modelType`, e.g., <<graph-sage, GraphSAGE>>. Dependent on the model type, there are additional fields.
