...
Running this mode results in a regression model of type `NodeRegression`, which is then stored in the <<model-catalog-ops, model catalog>>.
The regression model can be <<noderegression-pipelines-predict, applied>> on a graph to predict property values for new nodes.

More precisely, the training proceeds as follows:

. Apply `nodeLabels` and `relationshipType` filters to the graph.
. Apply the node property steps, added according to <<noderegression-pipelines-adding-node-properties, Adding node properties>>, on the whole graph.
. Select node properties to be used as features, as specified in <<noderegression-pipelines-adding-features, Adding features>>.
. Split the input graph into two parts: the train graph and the test graph. This is described in <<noderegression-pipelines-configure-splits, Configuring the node splits>>.
...
The number of folds `k` can be configured as described in <<noderegression-pipelines-configure-splits, Configuring the node splits>>.
. Each model candidate defined in the <<noderegression-pipelines-adding-model-candidates,parameter space>> is trained on each train set and evaluated on the respective validation set for every fold. The evaluation uses the specified primary metric.
...
include::../../../algorithms/common-configuration/common-parameters.adoc[]
...
include::../../pipeline-training-logging-note.adoc[]
...
. <<noderegression-pipelines-examples-create, Create>>
. <<noderegression-pipelines-examples-add-node-property, Add node properties>>
. <<noderegression-pipelines-examples-select-features, Select features>>
. <<noderegression-pipelines-examples-configure-split, Configure split>>
. <<noderegression-pipelines-examples-add-model-candidates, Adding model candidates>>
. <<noderegression-pipelines-examples-autotuning, Configure autotuning>>
