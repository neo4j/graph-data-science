...
// FIXME insert link to prediction
Moreover, the node property steps that are added to the training pipeline will be executed both when <<noderegression-pipelines-train,training>> a model and when the regression pipeline is applied for regression.
...
We now reconfigured the splitting of the graph for the pipeline, which will be applied during training.
--
