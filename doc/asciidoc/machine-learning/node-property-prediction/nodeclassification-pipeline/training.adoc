...
** `F1_MACRO`
** `ACCURACY`
** `OUT_OF_BAG_ERROR` (only for RandomForest and only gives validation score)
...
RETURN
  ...
  modelSelectionStats.bestTrial as bestTrial
----

.Results
[opts="header", cols="8, 2, 2, 2, 2"]
|===
| winningModel                                                                                                                                | avgTrainScore  | outerTrainScore    | testScore   | bestTrial
| {maxEpochs=100, minEpochs=1, penalty=0.0, patience=1, methodName=LogisticRegression, batchSize=100, tolerance=0.001, learningRate=0.001} | 0.999999989939394 | 0.9999999912121211 | 0.999999985 | 1
|===
