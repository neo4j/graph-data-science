Node {pipeline-type} Pipelines manage splitting the nodes into several sets for training, testing and validating the models defined in the <<{parameterspace-link},parameter space>>.
