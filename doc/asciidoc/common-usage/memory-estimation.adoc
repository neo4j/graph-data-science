...
Whenever any gds algorithm is executed, including graph creation, an estimation is always computed prior to the execution.
If the lower memory bound exceeds the current free jvm memory, execution is going to be blocked with an error containing information free memory and the estimated required memory.
...
Thus, it is still useful to run manual estimation before any large scale work loads.
