...
In short, there are three main execution modes to consider: `stream`, `write`, and `stats`, which we cover in this chapter.
...
The `stats` mode returns statistical results for the algorithm computation.
These may include counts or a percentile distribution for the algorithm results.
The same results can be acquired from the `write` mode of the procedure, but an algorithm in `stats` mode makes no modifications to the underlying Neo4j graph.
