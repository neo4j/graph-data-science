...
* For <<native-projection, native projections>>, the minimum page cache size can be roughly estimated by `8KB * 100 * readConcurrency`.
* For <<cypher-projection, Cypher projections>>, a higher page cache is required depending on the query complexity.

[NOTE]
====
Decreasing the page cache size is *not* recommended if the Neo4j instance runs both, operational and analytical workloads at the same time.
