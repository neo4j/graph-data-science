...
In order to make use of certain features of the GDS library, additional, configuration options have to be present.
These configuration parameters must be set in the `neo4j.conf` configuration file before starting the DBMS.
The following features will require such specific configuration options:
...
=== Model persistence
The <<model-catalog-store-ops, model store feature>> requires the configuration parameter `gds.model.store_location` to point to a directory in which the models will be stored.
This directory has to be writable by the Neo4j process.
