...
After the GDS workload has finished on a detached machine (for cases <<one, 1.>> and <<two, 2.>>) it now contains out-of-sync results written to its copied version of the graph from the Neo4j Causal Cluster.
