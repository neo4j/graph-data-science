...
If the procedure whitelist was configured make sure to include the GDS library or any procedure you need to execute
----
...
3. Check if the procedure whitelist is enabled int he `$NEO4J_HOME/conf/neo4j.conf` file and add the GDS library if necessary:
+
----
dbms.security.procedures.whitelist=gds.*
----
