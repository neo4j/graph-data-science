...
If the Typed projection is not expressive enough to describe the in-memory graph, we can instead use Cypher queries to select nodes and relationships.
...
For production, it is recommended to adapt the domain model in a way that it can take advantage of the loading speed of typed projections.
...
Similar to the default typed projection, we can load node and relationship properties using a Cypher projection.
...
The syntax is identical to the property mappings used in the typed projection.
