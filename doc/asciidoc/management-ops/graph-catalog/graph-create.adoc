...
As mentioned in the <<graph-create-native-syntax, syntax section>>, the `validateRelationships` flag controls whether an error will be raised when attempting to project a relationship where either the source or target node is not present in the <<node-projection-syntax, node projection>>.
