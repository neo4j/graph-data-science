...
--
.Project a `readSums` graph containing `Person` and `Book` nodes and aggregated `READ` relationships:
[source, cypher, role=noplay]
----
...
[role=query-example, group=count-aggregate]
--
.Stream the relationship property `numberOfReads` of the projected `readSums` graph:
[source, cypher, role=noplay]
----
