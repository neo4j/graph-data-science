...
As mentioned in the <<graph-create-native-syntax, syntax section>>, the `validateRelationships` flag controls whether an error will be raised when attempting to create a relationship that connects to or from a node not present in the <<node-projection-syntax, node projection>>.
Note that even if the flag is set to `false` such a relationship will still not be created but the loading process will continue with the next relationship instead.
