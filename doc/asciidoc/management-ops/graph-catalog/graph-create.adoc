...
// 2.1. very simple node properties only with String node property
...
==== Projecting single label and single relationship type

We are going to start with demonstrating how to load just a single `label` and `relationship`.
...
In the example above we projected an in-memory graph containing only the `Person` nodes with their `KNOWS` relationships.
We used the `short-hand` syntax to project the Node Label (`Person`) and Relationship Type (`KNOWS`) that we are interested in.
The used projections have been expanded to the full `Map` syntax as it can be seen from the `Results` table.
In addition, we can see that only the three `Person` nodes, and the two `KNOWS` relationships have been projected to the in-memory graph.


// 1. the very simple case, List of Strings --> Labels & Rels, no properties
==== Projecting multiple labels and multiple relationship types
...
// TODO: Add analysis on the results and some explanation to the query.
