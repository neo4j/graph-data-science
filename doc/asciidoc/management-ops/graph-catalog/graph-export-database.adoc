...
If we want to export `my-in-memory-graph` but additionally add the `myproperty` properties from `my-db-graph` we could use the `additionalProperties` configuration parameter.
