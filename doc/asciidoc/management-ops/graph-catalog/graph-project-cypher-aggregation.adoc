...
----
RETURN gds.alpha.graph(
    graphName: String,
    sourceNode: Node,
    targetNode: Node,
    sourceNodeProperties: Map,
    targetNodeProperties: Map,
...
| targetNode              | no       | The target node of the relationship. The targetNode can be null (for example due to an `OPTIONAL MATCH`), in which case the source node is projected as an unconnected node.
| sourceNodeProperties    | yes      | Properties to project on to the source node.
| targetNodeProperties    | yes      | Properties to project on to the target node.
| relationshipProperties  | yes      | Properties to project on to the relationship.
|===
...
WITH gds.alpha.graph('persons', source, target) AS g
RETURN
...
At its current state, Cypher Aggregation does not support graphs with multiple node labels or relationship types.
...
WITH gds.alpha.graph(
  'graphWithProperties',
  source,
  target,
  source { age: coalesce(source.age, 18), price: coalesce(source.price, 5.0), .ratings },
  target { age: coalesce(target.age, 18), price: coalesce(target.price, 5.0), .ratings }
) as g
...
Analogous to node properties, we can project relationship properties using the sixth parameter.
If we only want to project relationship properties and not any node properties, we must provide two `null` values for the sourceNodeProperties and targetNodeProperties parameters respectively.
...
WITH gds.alpha.graph('readCount', source, target, null, null, { numberOfReads: numberOfReads }) AS g
RETURN
  g.graphName AS graph, g.nodeCount AS nodes, g.relationshipCount AS rels
----
...
WITH gds.alpha.graph('readSums', source, target, null, null, { numberOfPages: numberOfPages }) AS g
RETURN
  g.graphName AS graph, g.nodeCount AS nodes, g.relationshipCount AS rels
----
...
WITH gds.alpha.graph('existingNumberOfPages', source, target, null, null, r { .numberOfPages }) AS g
RETURN
  g.graphName AS graph, g.nodeCount AS nodes, g.relationshipCount AS rels
----
