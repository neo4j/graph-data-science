...
[NOTE]
====
Node identifiers are represented by 64-Bit signed long values.
If the input data contains very large node id values, one might observe a high memory footprint for the projected graph.
In these situations, the memory footprint of the graph can be reduced by switching to another <<sharded-id-map-feature-toggle, id map implementation>>
====
...
The <<arrow-client-server-protocol>> can also be used to create a new Neo4j database instead of an in-memory graph.
...
| debug_log         | Boolean           | Yes       | False                 | Write the database creation process into the log.
...
See <<arrow-send-nodes>> and <<arrow-send-relationships>> for further details.
