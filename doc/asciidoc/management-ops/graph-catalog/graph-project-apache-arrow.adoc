...
The action type is called `CREATE_GRAPH` and the action body contains configures the import process.
...
To increase the throughput, Flight streams can be sent from multiple clients in parallel.
