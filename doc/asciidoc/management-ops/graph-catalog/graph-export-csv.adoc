...
.Export the `my-in-memory-graph` from GDS with the `my-property` from `my-db-graph` into a directory `my-export`:
[source, cypher, role=noplay]
----
CALL gds.graph.export('my-graph', { exportName: 'my-export', additionalNodeProperties: ['name']})
----
