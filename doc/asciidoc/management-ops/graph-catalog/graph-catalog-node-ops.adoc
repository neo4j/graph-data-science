...
In the following example, we will only remove the scores from the projected `Person` nodes.

[role=query-example]
--
.Remove the `score` property from the projected `Person` nodes:
[source, cypher, role=noplay]
----
CALL gds.graph.removeNodeProperties('socialGraph', ['score'], ['Person'])
YIELD propertiesRemoved
----
