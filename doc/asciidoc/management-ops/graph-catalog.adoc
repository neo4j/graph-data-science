...
The new database can be created and started using link:https://neo4j.com/docs/cypher-manual/current/administration/databases/#administration-databases-start-database[`databases management commands`].
...
.Export a named graph to a new databases in the Neo4j databases direct:
[source,cypher]
----
CALL gds.beta.graph.export('my-graph', {
   dbName: 'my-graph-db'
})
----
