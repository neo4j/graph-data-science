...
YIELD graphName, nodeQuery, relationshipQuery, nodeCount, relationshipCount, creationTime, modificationTime, sizeInBytes, memoryUsage;
----
...
The result field `sizeInBytes` yields the number of bytes used in the Java Heap to store that graph.
The result field `memoryUsage` is the same information in a human readable format.
...
The above example requires all given properties to be present on at least one node projection.
The procedure can be configured to remove just the properties for a specific node projection.
In the following example, we ran an algorithm on a sub-graph and want to write those results to Neo4j.
...
The procedure will fail for properties that are not mapped to the given projection.
If no label or `*` is given, all projections that have all the given properties will be considered.
...
The above example requires all given properties to be present on at least one node projection.
The procedure can be configured to write just the properties for a specific node projection.
In the following example, we ran an algorithm on a sub-graph and want to write those results to Neo4j.
