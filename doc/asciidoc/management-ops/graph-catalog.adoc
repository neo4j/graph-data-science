...
YIELD graphName, nodeQuery, relationshipQuery, nodeCount, relationshipCount, createdTime;
----

The information contains basic statistics about the graph, e.g., the node and relationship count.
The result field `createdTime` indicates when the graph was created in memory.
