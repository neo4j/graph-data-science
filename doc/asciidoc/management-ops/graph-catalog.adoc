...
| nodeProjection         | Map           | The node projection(s) used for native creation.
| relationshipProjection | Map           | The relationship projection(s) used for native creation.
| nodeQuery              | String        | The node query used to create the graph.
| relationshipQuery      | String        | The relationship query used to create the graph.
| nodeCount              | Integer       | The number of nodes in the graph.
| relationshipCount      | Integer       | The number of relationships in the graph.
| schema                 | Map           | The schema of the graph, see below.
| degreeDistribution     | Map           | Statistical information about the degrees of nodes in the graph.
| creationTime           | LocalDateTime | The time at which the graph was originally created in memory.
| modificationTime       | LocalDateTime | The time at which the graph was updated by an algorithm running in `mutate` mode.
| sizeInBytes            | Integer       | The number of bytes used in the Java Heap to store that graph.
| memoryUsage            | String        | The same information as `sizeInBytes` but in a human readable format.
|===

The `nodeProjection` and `relationshipProjection` columns are primarily applicable to <<native-projection, native projection>>).
For cypher created graphs, the projections are constructed after the fact in a best effort manner.
...
The `schema` consists of information about the available node and relationship schema.
This information is represented as a map with keys `nodes` and `relationships`.
For each node label, `nodes` maps the label to its property keys along with the data type of the property.
Similarly, `relationships` maps the relationship types to their property names and data types.
