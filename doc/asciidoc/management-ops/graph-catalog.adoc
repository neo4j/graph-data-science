...
YIELD graphName, nodeQuery, relationshipQuery, nodeCount, relationshipCount, creationTime;
----
...
We can write multiple node properties stored in a named in-memory graph back to Neo4j.
This is useful if we ran multiple algorithms in `mutate` mode and want to write back all results at once.
...
This can be used to write algorithm results, e.g. for node similarity, or relationships that have been aggregated during graph creation.
...
Note, that the above example does not write relationship properties by default.
If we want to write properties, we need to specify the relationship property explicitly.
