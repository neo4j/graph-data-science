...
public class ClosenessCentralityStreamProc {
    ...
    public Stream<CentralityStreamResult> stream(
        ...
        return facade.algorithms().centrality().closenessCentralityStream(graphName, configuration);
    }

    @Procedure(name = "gds.closeness.stream.estimate", mode = READ)
    @Description(MEMORY_ESTIMATION_DESCRIPTION)
    public Stream<MemoryEstimateResult> estimate(
        @Name(value = "graphNameOrConfiguration") Object graphNameOrConfiguration,
        @Name(value = "algoConfiguration") Map<String, Object> algoConfiguration
    ) {
        return facade.algorithms().centrality().closenessCentralityStreamEstimate(graphNameOrConfiguration, algoConfiguration);
    }

    @Deprecated(forRemoval = true)
    @Internal
    @Procedure(value = "gds.beta.closeness.stream", mode = READ, deprecatedBy = "gds.closeness.stream")
    @Description(CLOSENESS_DESCRIPTION)
    public Stream<CentralityStreamResult> streamBeta(
        @Name(value = "graphName") String graphName,
        @Name(value = "configuration", defaultValue = "{}") Map<String, Object> configuration
    ) {
