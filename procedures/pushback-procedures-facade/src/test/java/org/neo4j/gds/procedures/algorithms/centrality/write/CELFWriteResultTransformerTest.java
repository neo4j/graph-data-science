...
class CELFWriteResultTransformerTest {

    @Test
    void shouldTransform() {
        var config = Map.of("a", (Object) ("foo"));
        var result = mock(CELFResult.class);
        when(result.totalSpread()).thenReturn(44d);
        var graph = mock(Graph.class);
        when(graph.nodeCount()).thenReturn(11L);

        var writeStep = mock(CelfWriteStep.class);
        ...
        var transformer = new CELFWriteResultTransformer(
            graph,
        ...
        assertThat(writeResult.findFirst().orElseThrow())
            .satisfies(write -> {
                ...
                assertThat(write.totalSpread()).isEqualTo(44d);
                assertThat(write.nodePropertiesWritten()).isEqualTo(42L);
                assertThat(write.nodeCount()).isEqualTo(11L);
            });
    }
