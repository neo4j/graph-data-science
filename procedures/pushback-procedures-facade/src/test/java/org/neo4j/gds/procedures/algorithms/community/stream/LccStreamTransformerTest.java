...
class LccStreamTransformerTest {

    @Test
    void shouldTransformResult(){

        var result = mock(LocalClusteringCoefficientResult.class);
        when(result.localClusteringCoefficients()).thenReturn(HugeDoubleArray.of(10,20));
        ...
        var transformer = new LccStreamTransformer(graphMock);

        var stream = transformer.apply(new TimedAlgorithmResult<>(result, -1));
        assertThat(stream).containsExactlyInAnyOrder(
            new LocalClusteringCoefficientStreamResult(0, 10L),
            new LocalClusteringCoefficientStreamResult(1, 20L)
        );
    ...
    void shouldTransformEmptyResult(){
        ...
        var stream = transformer.apply(new TimedAlgorithmResult<>(LocalClusteringCoefficientResult.EMPTY,-1));
