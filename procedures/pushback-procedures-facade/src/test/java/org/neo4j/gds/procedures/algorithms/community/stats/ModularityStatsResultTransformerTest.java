...
class ModularityStatsResultTransformerTest {
    ...
    void shouldTransformResult(){
        ...
        when(result.communityCount()).thenReturn(100L);
        when(result.relationshipCount()).thenReturn(30L);
        when(result.nodeCount()).thenReturn(20L);
        when(result.totalModularity()).thenReturn(55d);

        var transformer = new ModularityStatsResultTransformer(config);
