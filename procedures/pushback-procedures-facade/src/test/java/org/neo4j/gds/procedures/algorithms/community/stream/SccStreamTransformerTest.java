...
class SccStreamTransformerTest {

    @Test
    void shouldTransformResult(){

        var result = HugeLongArray.of(10L,20L);
        ...
        var transformer = new SccStreamTransformer(graphMock,new Concurrency(1),true);

        var stream = transformer.apply(new TimedAlgorithmResult<>(result, -1));
        assertThat(stream).containsExactlyInAnyOrder(
            new SccStreamResult(0, 0L),
            new SccStreamResult(1, 1L)
        );
    ...
    void shouldTransformEmptyResult(){
        ...
        var stream = transformer.apply(new TimedAlgorithmResult<>(HugeLongArray.newArray(0),-1));
