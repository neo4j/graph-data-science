...
class ModularityOptimizationStreamTransformerTest {

    @Test
    void shouldTransformResult(){

        var result = mock(ModularityOptimizationResult.class);
        when(result.asNodeProperties()).thenReturn(NodePropertyValuesAdapter.adapt(HugeLongArray.of(0L,20L,10L,20L,10L,10L)));
        ...
        var transformer = new ModularityOptimizationStreamTransformer(graphMock,new Concurrency(1), Optional.of(2L),true);

        var stream = transformer.apply(new TimedAlgorithmResult<>(result, -1));
        assertThat(stream).containsExactlyInAnyOrder(
            new ModularityOptimizationStreamResult(1,0),
            new ModularityOptimizationStreamResult(2,1),
            new ModularityOptimizationStreamResult(3,0),
            new ModularityOptimizationStreamResult(4,1),
            new ModularityOptimizationStreamResult(5,1)
        );
    ...
    void shouldTransformEmptyResult(){
        ...
        var stream = transformer.apply(new TimedAlgorithmResult<>(ModularityOptimizationResult.EMPTY,-1));
