...
class ApproxMaxKCutStreamTransformerTest {

    @Test
    void shouldTransformResult(){

        var result = mock(ApproxMaxKCutResult.class);
        when(result.candidateSolution()).thenReturn(HugeByteArray.of((byte) 0,(byte) 2,(byte)1,(byte)2,(byte)1,(byte)1));
        ...
        var transformer = new ApproxMaxKCutStreamTransformer(graphMock,new Concurrency(1), Optional.of(2L));

        var stream = transformer.apply(new TimedAlgorithmResult<>(result, -1));
        assertThat(stream).containsExactlyInAnyOrder(
            new ApproxMaxKCutStreamResult(1,2),
            new ApproxMaxKCutStreamResult(2,1),
            new ApproxMaxKCutStreamResult(3,2),
            new ApproxMaxKCutStreamResult(4,1),
            new ApproxMaxKCutStreamResult(5,1)
        );
    ...
    void shouldTransformEmptyResult(){
        ...
        var stream = transformer.apply(new TimedAlgorithmResult<>(ApproxMaxKCutResult.EMPTY, -1));
