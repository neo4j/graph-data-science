...
class KnnStatsResultTransformerTest {
    ...
    void shouldComputeResultWithDistribution(){
        ...
        var knnResult = mock(KnnResult.class);
        when(knnResult.nodesCompared()).thenReturn(1L);
        when(knnResult.totalSimilarityPairs()).thenReturn(3L);
        when(knnResult.nodePairsConsidered()).thenReturn(100L);

        when(knnResult.streamSimilarityResult()).thenReturn(
            Stream.of(
        ...
        var transformer = new KnnStatsResultTransformer(
            graph,
            true,
            config,
    ...
    void shouldComputeResultForEmpty(){
        ...
        var statsResult = transformer.apply(new TimedAlgorithmResult<>(KnnResult.EMPTY,10));
        assertThat(statsResult.findFirst().orElseThrow())
            .satisfies(stats -> {
                assertThat(stats.preProcessingMillis()).isEqualTo(0);
                assertThat(stats.computeMillis()).isEqualTo(10);
