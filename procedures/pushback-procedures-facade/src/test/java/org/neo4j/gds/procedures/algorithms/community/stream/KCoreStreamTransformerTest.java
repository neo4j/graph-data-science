...
class KCoreStreamTransformerTest {

    @Test
    void shouldTransformResult(){

        var result = mock(KCoreDecompositionResult.class);
        when(result.coreValues()).thenReturn(HugeIntArray.of(10,20));
        ...
        var transformer = new KCoreStreamTransformer(graphMock);

        var stream = transformer.apply(new TimedAlgorithmResult<>(result, -1));
        assertThat(stream).containsExactlyInAnyOrder(
            new KCoreDecompositionStreamResult(0, 10L),
            new KCoreDecompositionStreamResult(1, 20L)
        );
    ...
    void shouldTransformEmptyResult(){
        ...
        var stream = transformer.apply(new TimedAlgorithmResult<>(KCoreDecompositionResult.EMPTY, -1));
