...
class WccStreamTransformerTest {


    @Test
    void shouldTransformResult(){

        var result = mock(HugeAtomicDisjointSetStruct.class);
        ...
        var transformer = new WccStreamTransformer(graphMock,new Concurrency(1), Optional.of(2L),true);

        var stream = transformer.apply(new TimedAlgorithmResult<>(result, -1));
        assertThat(stream).containsExactlyInAnyOrder(
            new WccStreamResult(1,0),
            new WccStreamResult(2,1),
            new WccStreamResult(3,0),
            new WccStreamResult(4,1),
            new WccStreamResult(5,1)
        );
    ...
    void shouldTransformEmptyResult(){
        ...
        var emptyResult = new HugeAtomicDisjointSetStruct(
            0,
            new Concurrency(1)
        );
        var stream = transformer.apply(new TimedAlgorithmResult<>(emptyResult,-1L));
