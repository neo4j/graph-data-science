...
class KMeansStreamTransformerTest {

    @Test
    void shouldTransformResultWithoutSilhouette(){

        var result  = new KmeansResult(
            HugeIntArray.of(10,11),
            HugeDoubleArray.of(200,300),
            new double[0][0],
            0,
            null,
            0
        ...
        var transformer = new KMeansStreamTransformer(graphMock);

        var stream = transformer.apply(new TimedAlgorithmResult<>(result, -1));
        assertThat(stream).containsExactlyInAnyOrder(
            new KMeansStreamResult(0, 10L,200,-1),
            new KMeansStreamResult(1, 11L,300,-1)
    ...
    @Test
    void shouldTransformResultWithSilhouette(){
        ...
        var stream = transformer.apply(new TimedAlgorithmResult<>(result, -1));
        assertThat(stream).containsExactlyInAnyOrder(
            new KMeansStreamResult(0, 10L,200,400),
            new KMeansStreamResult(1, 11L,300,500)
    ...
    void shouldTransformEmptyResult(){
        ...
        var stream = transformer.apply(new TimedAlgorithmResult<>(KmeansResult.empty(3), -1));
