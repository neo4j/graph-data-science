...
import org.neo4j.gds.procedures.algorithms.centrality.CentralityDistributionInstructions;
...
public class PushbackCentralityStatsProcedureFacade {
    ...
    public Stream<HitsStatsResult> hits(String graphName, Map<String, Object> configuration) {
        ...
        return businessFacade.hits(
            ...
            config.jobId(),
            config.logProgress(),
            graphResources -> new HitsStatResultTransformer(
                config.toMap()
            )
        ).join();
    }
