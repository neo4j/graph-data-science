...
public class AlphaHarmonicStreamResultTransformer implements ResultTransformer<TimedAlgorithmResult<HarmonicResult>, Stream<AlphaHarmonicStreamResult>> {
    ...
    public AlphaHarmonicStreamResultTransformer(Graph graph) {
        this.graph = graph;
    }

    @Override
    public Stream<AlphaHarmonicStreamResult> apply(TimedAlgorithmResult<HarmonicResult> timedAlgorithmResult) {

        var harmonicResult = timedAlgorithmResult.result();

        var nodePropertyValues = harmonicResult.nodePropertyValues();

        return LongStream.range(IdMap.START_NODE_ID, nodePropertyValues.nodeCount())
