...
class NeighbourFinderTest extends BaseTest {
    ...
    void excludeDirectRelationships() {
        ...
        runInFullAccessTransaction(db, tx -> {
            ...
            Set<Node> neighbors = neighbourFinder.findCommonNeighbours(node1, node2, null, Direction.BOTH);
    ...
    @Test
    void sameNodeHasNoCommonNeighbours() {
        runInFullAccessTransaction(db, tx -> {
            Node node1 = tx.createNode();
            Node node2 = tx.createNode();
            node1.createRelationshipTo(node2, FRIEND);
        ...
        runInFullAccessTransaction(db, tx -> {
            Node node1 = tx.getNodeById(0);
            Set<Node> neighbors = neighbourFinder.findCommonNeighbours(node1, node1, null, Direction.BOTH);
    ...
    @Test
    void findNeighboursExcludingDirection() {
        runInFullAccessTransaction(db, tx -> {
            Node node1 = tx.createNode();
            Node node2 = tx.createNode();
    ...
    void findOutgoingNeighbors() {
        ...
        runInFullAccessTransaction(db, tx -> {
            Node node1 = tx.getNodeById(0);
            Node node2 = tx.getNodeById(1);
            Set<Node> neighbors = neighbourFinder.findCommonNeighbours(node1, node2, FOLLOWS, Direction.OUTGOING);
    ...
    void findIncomingNeighbors() {
        ...
        runInFullAccessTransaction(db, tx -> {
            Node node1 = tx.getNodeById(0);
            Node node2 = tx.getNodeById(1);
            Set<Node> neighbors = neighbourFinder.findCommonNeighbours(node1, node2, FOLLOWS, Direction.INCOMING);
    ...
    @Test
    void findNeighboursOfSpecificRelationshipType() {
        runInFullAccessTransaction(db, tx -> {
            Node node1 = tx.createNode();
            Node node2 = tx.createNode();
            Node node3 = tx.createNode();
        ...
        runInFullAccessTransaction(db, tx -> {
            Node node1 = tx.getNodeById(0);
            Node node2 = tx.getNodeById(1);
            Set<Node> neighbors = neighbourFinder.findCommonNeighbours(node1, node2, COLLEAGUE, Direction.BOTH);
    ...
    void dontCountDuplicates() {
        runInFullAccessTransaction(db, tx -> {
            ...
            var neighbourFinder = new NeighbourFinder();
            var neighbours = neighbourFinder.findNeighbours(nodes[0], nodes[1], null, Direction.BOTH);

            assertEquals(2, neighbours.size());
            assertThat(neighbours, IsIterableContaining.hasItems(nodes[2], nodes[3]));
        });
    }
    ...
    void otherNodeCountsAsNeighbor() {
        runInFullAccessTransaction(db, tx -> {
            ...
            assertEquals(2, neighbours.size());
            assertThat(neighbours, IsIterableContaining.hasItems(nodes[0], nodes[1]));
        });
    ...
    void otherNodeCountsAsOutgoingNeighbor() {
        ...
        runInFullAccessTransaction(db, tx -> {
            ...
            var neighbourFinder = new NeighbourFinder();
            var neighbours = neighbourFinder.findNeighbours(nodes[0], nodes[1], null, Direction.OUTGOING);

            assertEquals(1, neighbours.size());
            assertThat(neighbours, IsIterableContaining.hasItems(nodes[1]));
        });
    }
}
