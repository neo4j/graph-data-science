...
class HDBScanWriteProcTest extends BaseProcTest {
    ...
    void write() {
        ...
        assertThat(rowCount)
            .as("Write mode should only have a single row.")
            .isEqualTo(1);

        runQueryWithRowConsumer(
            "MATCH (n:Node) RETURN count(n.clusterId) AS nodeCount, count(distinct(n.clusterId)) AS numberOfClusters",
            row -> {
                assertThat(row.getNumber("nodeCount")).asInstanceOf(LONG).isEqualTo(10);
                assertThat(row.getNumber("numberOfClusters")).asInstanceOf(LONG).isEqualTo(2);
            }
        );
    }
}
