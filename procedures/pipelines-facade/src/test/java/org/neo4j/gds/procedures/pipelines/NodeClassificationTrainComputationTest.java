...
class NodeClassificationTrainComputationTest {
    @Test
    void shouldValidateContextNodeLabelsInNodePropertySteps() {
        var pipelineRepository = mock(PipelineRepository.class);
        var nodeClassificationTrainComputation = constructNodeClassificationTrainComputation(pipelineRepository);
        ...
        when(pipelineRepository.getNodeClassificationTrainingPipeline(
            User.DEFAULT,
        ...
        assertThatIllegalArgumentException()
            .isThrownBy(() -> nodeClassificationTrainComputation.compute(null, graphStore))
            .withMessage(
    ...
    private static NodeClassificationTrainComputation constructNodeClassificationTrainComputation(PipelineRepository pipelineRepository) {
        Map<String, Object> rawConfiguration = Map.of(
            "graphName", "some graph name",
            "metrics", List.of("OUT_OF_BAG_ERROR"),
            "modelName", "some model name",
            "pipeline", "some pipeline",
            "targetProperty", "some target property"
        );

        var configuration = NodeClassificationPipelineTrainConfig.of(
