...
public class MCMFResultBuilderForMutateMode implements ResultBuilder<MCMFMutateConfig, CostFlowResult, MCMFMutateResult, RelationshipsWritten> {


    @Override
    public MCMFMutateResult build(
        Graph graph,
        ...
        if (result.isEmpty()) {
            return MCMFMutateResult.emptyFrom(timings, configuration.toMap());
        }

        var flowResult = result.get();

        return new MCMFMutateResult(
