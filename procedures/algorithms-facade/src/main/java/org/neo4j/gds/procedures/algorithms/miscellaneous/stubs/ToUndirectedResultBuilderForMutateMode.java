...
class ToUndirectedResultBuilderForMutateMode implements ResultBuilder<ToUndirectedConfig, SingleTypeRelationships, ToUndirectedMutateResult, RelationshipsWritten> {
    @Override
    public ToUndirectedMutateResult build(
        Graph graph,
        ToUndirectedConfig configuration,
        Optional<SingleTypeRelationships> result,
        AlgorithmProcessingTimings timings,
        Optional<RelationshipsWritten> metadata
    ) {
        if (result.isEmpty()) return ToUndirectedMutateResult.emptyFrom(timings, configuration.toMap());
