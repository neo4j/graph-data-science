...
class CollapsePathResultBuilderForMutateMode implements ResultBuilder<CollapsePathConfig, SingleTypeRelationships, CollapsePathMutateResult, Void> {
    ...
    public CollapsePathMutateResult build(
        ...
        if (result.isEmpty()) return CollapsePathMutateResult.emptyFrom(timings, configuration.toMap());

        var singleTypeRelationships = result.get();

        return new CollapsePathMutateResult(
            timings.preProcessingMillis,
            timings.computeMillis,
            timings.sideEffectMillis,
            singleTypeRelationships.count(),
            configuration.toMap()
        );
    }
}
