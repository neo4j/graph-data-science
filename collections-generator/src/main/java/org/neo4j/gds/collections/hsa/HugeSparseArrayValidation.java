...
class HugeSparseArrayValidation {

    private final Messager messager;

    HugeSparseArrayValidation(Messager messager) {
        this.messager = messager;
    }

    Optional<Element> validate(Element element) {
        if (!hasValidEnclosingElements(element)) {
            return Optional.empty();
        }
        return Optional.of(element);
    }

    private boolean hasValidEnclosingElements(Element element) {
