...
class LongArrayImplTest {

    @Test
    void longArrayValue() {
        var value = new LongArrayImpl(new long[]{1, 2, 3, 4, 5, 6, 7, 8, 9});
        assertThat(value.longArrayValue()).isEqualTo(new long[]{1, 2, 3, 4, 5, 6, 7, 8, 9});
    }

    @Test
    void longValue() {
        var value = new LongArrayImpl(new long[]{1, 2, 3});
    ...
    void testEquals() {
        ...
        assertThat(new LongArrayImpl(new long[]{})).isEqualTo(PrimitiveValues.EMPTY_LONG_ARRAY);

        var value = new LongArrayImpl(new long[]{1, 2, 3});
        // equal to another instance if the held values are equal
        assertThat(value).isEqualTo(new LongArrayImpl(new long[]{1, 2, 3}));
        // equal to an array if the held array is equal
        assertThat(value.equals(new long[]{1, 2, 3})).isTrue();
    }
}
