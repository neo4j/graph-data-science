...
class ShortLongArrayImplTest {

    @Test
    void longArrayValue() {
        var value = new ShortLongArrayImpl(new short[]{1, 2, 3, 4, 5, 6, 7, 8, 9});
        assertThat(value.longArrayValue()).isEqualTo(new long[]{1, 2, 3, 4, 5, 6, 7, 8, 9});
    }

    @Test
    void longValue() {
        var value = new ShortLongArrayImpl(new short[]{1, 2, 3});
    ...
    void testEquals() {
        ...
        assertThat(new ShortLongArrayImpl(new short[]{})).isEqualTo(PrimitiveValues.EMPTY_LONG_ARRAY);

        var value = new ShortLongArrayImpl(new short[]{1, 2, 3});
        // equal to another instance if the held values are equal
        assertThat(value).isEqualTo(new ShortLongArrayImpl(new short[]{1, 2, 3}));
        // equal to an array if the held array is equal
        assertThat(value.equals(new short[]{1, 2, 3})).isTrue();
    }
}
