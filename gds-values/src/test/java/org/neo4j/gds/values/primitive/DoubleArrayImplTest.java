...
class DoubleArrayImplTest {
    ...
    void doubleArrayValue() {
        var value = new DoubleArrayImpl(new double[]{1, 2, 3, 4, 5, 6, 7, 8, 9});
        assertThat(value.doubleArrayValue()).isEqualTo(new double[]{1, 2, 3, 4, 5, 6, 7, 8, 9});
    }

    @Test
    void longValue() {
        var value = new DoubleArrayImpl(new double[]{1, 2, 3});
    ...
    void type() {
        var value = new DoubleArrayImpl(new double[]{1, 2, 3});
        assertThat(value.type()).isEqualTo(ValueType.DOUBLE_ARRAY);
    }

    @Test
    void testEquals() {
        // empty is equal to the empty constant
        assertThat(new DoubleArrayImpl(new double[]{})).isEqualTo(PrimitiveValues.EMPTY_LONG_ARRAY);

        var value = new DoubleArrayImpl(new double[]{1, 2, 3});
        // equal to another instance if the held values are equal
        assertThat(value).isEqualTo(new DoubleArrayImpl(new double[]{1, 2, 3}));
