...
public class ReducedSoftmax extends SingleParentVariable<Matrix, Matrix> {
    ...
    public Matrix gradientForParent(ComputationContext ctx) {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                int indexOffset = row * cols;
                var indexToUpdate = indexOffset + col;
                var softmaxData = selfData.dataAt(indexToUpdate);
                for (int softmaxCol = 0; softmaxCol < cols; softmaxCol++) {
                    double valueAtOtherColumn = selfData.dataAt(indexOffset + softmaxCol);
                    double gradientAtOtherColumn = selfGradient.dataAt(indexOffset + softmaxCol);
