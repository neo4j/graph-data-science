...
public final class InheritedComputationStreamProc extends BaseProc {
    @Procedure(
        name = "gds.pregel.inherited.test.stream",
        mode = Mode.READ
    )
    @Description("Test computation description")
    public Stream<PregelStreamResult> stream(@Name("graphName") String graphName,
            @Name(value = "configuration", defaultValue = "{}") Map<String, Object> configuration) {
        var specification = new InheritedComputationStreamSpecification();
        var executor = new ProcedureExecutor<>(specification, executionContext());
        return executor.compute(graphName, configuration);
    }

    @Procedure(
        name = "gds.pregel.inherited.test.stream.estimate",
        mode = Mode.READ
    )
