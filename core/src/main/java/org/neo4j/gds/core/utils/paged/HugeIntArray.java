 ...
 * <li>The array is not optimized for sparseness and has a large memory overhead if the values written to it are very sparse (see {@link org.neo4j.gds.collections.hsa.HugeSparseIntArray} for a different implementation that can profit from sparse data).</li>
