...
final class ConcurrencyConfig {
    ...
    static ConcurrencyConfig of() {
        Integer definedProcessors = Integer.getInteger(
            PROCESSORS_OVERRIDE_PROPERTY,
            Runtime.getRuntime().availableProcessors()
        );
        return new ConcurrencyConfig(definedProcessors, ConcurrencyMonitor.instance().isUnlimited());
    }

    /* test-private */ ConcurrencyConfig(int availableProcessors, boolean isOnEnterprise) {
        corePoolSize = isOnEnterprise ? availableProcessors : Math.min(availableProcessors, CONCURRENCY_LIMITATION);
    }
}
