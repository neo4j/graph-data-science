...
public abstract class HugeAtomicByteArray implements HugeCursorSupport<byte[]> {
    ...
    public static long memoryEstimation(long size) {
        ...
        long dataSize;
        if (size <= ArrayUtil.MAX_ARRAY_LENGTH) {
            instanceSize = sizeOfInstance(SingleHugeAtomicByteArray.class);
            dataSize = sizeOfByteArray((int) size);
        } else {
            instanceSize = sizeOfInstance(PagedHugeAtomicByteArray.class);
    ...
    static final class PagedHugeAtomicByteArray extends HugeAtomicByteArray {
        ...
        private static long memoryUsageOfData(long size) {
            ...
            long bytesPerPage = sizeOfByteArray(PAGE_SIZE);
            int sizeOfLastPage = exclusiveIndexOfPage(size);
            long bytesOfLastPage = sizeOfByteArray(sizeOfLastPage);
