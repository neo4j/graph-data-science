...
abstract class AbstractRecordBasedScanner<Reference, Record extends AbstractBaseRecord, Store extends RecordStore<Record>> implements StoreScanner<Reference> {
    ...
    private final class ScanCursor implements StoreScanner.ScanCursor<Reference> {
        ...
        private void loadAtOffset(int offset) throws IOException {
            do {
                ...
                Neo4jProxy.read(recordFormat, record, pageCursor, RecordLoad.CHECK, recordSize, recordsPerPage);
    ...
    AbstractRecordBasedScanner(int prefetchSize, SecureTransaction transaction) {
        ...
        this.maxId = 1L + Neo4jProxy.getHighestPossibleIdInUse(store, PageCursorTracer.NULL);
    ...
    public final StoreScanner.ScanCursor<Reference> getCursor(KernelTransaction transaction) {
        ...
        if (cursor == null) {
            ...
            try {
                if (pagedFile != null) {
                    pageCursor = Neo4jProxy.pageFileIO(pagedFile, next, PagedFile.PF_READ_AHEAD | PagedFile.PF_SHARED_READ_LOCK, transaction.pageCursorTracer());
                } else {
                    long recordId = next * (long) recordSize;
                    pageCursor = Neo4jProxy.openPageCursorForReading(store, recordId, transaction.pageCursorTracer());
                }
            } catch (IOException e) {
                throw new UncheckedIOException(e);
            }
