...
class CsvNodeVisitorTest extends CsvVisitorTest {
    ...
    void visitNodesWithoutLabelsAndProperties() {
        var nodeVisitor = new CsvNodeVisitor(tempDir, NodeSchema.builder().build(), false);
    ...
    void visitNodesWithProperties() {
        ...
        nodeVisitor.property("neoId", 42L);
        nodeVisitor.endOfEntity();

        nodeVisitor.id(1L);
        nodeVisitor.property("foo", 42.0);
        nodeVisitor.property("neoId", 43L);
        nodeVisitor.endOfEntity();

        nodeVisitor.id(2L);
        nodeVisitor.property("bar", 21.0);
        nodeVisitor.property("neoId", 44L);
        nodeVisitor.endOfEntity();

        nodeVisitor.close();

        assertCsvFiles(List.of("nodes_0.csv", "nodes_header.csv"));
        assertHeaderFile("nodes_header.csv", computeNodeSchema(nodeSchema, true).unionProperties());
        assertDataContent(
            "nodes_0.csv",
            List.of(
                List.of("0", "21.0", "42.0", "42"),
                List.of("1", "", "42.0", "43"),
                List.of("2", "21.0", "", "44")
            )
        );
    ...
    void visitNodesWithLabelsAndProperties() {
        ...
        var nodeSchema = NodeSchema.builder()
            ...
            .build();
        nodeSchema = computeNodeSchema(nodeSchema, true);
        var nodeVisitor = new CsvNodeVisitor(tempDir, nodeSchema, true);
        ...
        nodeVisitor.property("neoId", 45L);
        nodeVisitor.endOfEntity();
        ...
        nodeVisitor.property("bar", 21L);
        nodeVisitor.property("baz", 21.0);
        nodeVisitor.property("neoId", 46L);
        nodeVisitor.endOfEntity();
        ...
        assertDataContent(
            "nodes_A_B_0.csv",
            List.of(  //id   bar   baz     foo   neoId
                List.of("0", "21", "21.0", "42", "42"),
                List.of("4", "21", "21.0", "", "46")
            )
        );
        ...
        assertDataContent(
            "nodes_B_0.csv",
            List.of(  //id   bar    baz  neoId
                List.of("2", "21", "21.0", "44")
            )
        );
