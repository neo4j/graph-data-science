...
class HugeSerialObjectMergeSortTest {
    ...
    void sortArray(long size, boolean useSingleArray) {
        var array = useSingleArray
            ? HugeObjectArray.newSingleArray(Long.class, (int) size)
            : HugeObjectArray.newPagedArray(Long.class, size);
        var longs = ArrayUtils.toObject(new Random().longs(size, 0, size).toArray());
        var doubles = new Random().doubles(size).toArray();
        for (int i = 0; i < size; i++) {
            array.set(i, longs[i]);
        }

        HugeSerialObjectMergeSort.sort(Long.class, array, l -> doubles[(int) l.longValue()]);
